...
<td>
		  <a href="<?php echo $home_path ?>/reports/checkout/xt_viewOccupancyDetails-xls.php" style="margin:10px 0 0 65px;color:#000;font-size:13px;font-weight:bold;"><button type="button" id="pdf" style="" class="submitbtnprint btnn"><img src="../../images/excel1.png"  class="sbtBtnImg"/>&nbsp;Export&nbsp;</button></a>
...
if(isset($_GET['val'])){
	$item_where= " where arrival_date like '%".$_GET['val']."%' OR guest_name like '%".$_GET['val']."%' OR room_no like '%".$_GET['val']."%'";
$sql=mysql_query("select distinct guestreg_id,arrival_date,departure_date,room_no,guest_name,address1,address2,city,pin_code,nationality,purpose_visit from guest_register $item_where");
} else{ 
	$sql=mysql_query("select distinct guestreg_id,arrival_date,departure_date,room_no,guest_name,address1,address2,city,pin_code,nationality,purpose_visit from guest_register");
} 
	...
	<div style="margin: 21px 0 26px 10px;;width:95%;" class="alert alert-success">
                               You have not created any Occupancy details...
    </div>
<?php } ?>
</table>
	</div>
