...
if(fromdate!="" && todate!="")
{
document.location="outstanding_report.php?fromdate="+fromdate+"&todate="+todate;
}
...
<table class="table table-condensed table-hover table-striped table-bordered" cellpadding="0" cellspacing="0" border="0" class="table" style="margin:0 0 15px -5px;text-align:center;font-size:12px;">
	<tr class="info">
		...
		<td colspan="13" style="text-align:center;"><h3 style="text-align:center;font-size:14px;padding:10px;background:#0080C0;color:#fff;margin:1px 0 0 0;text-transform:uppercase;width:100%;"><b>Outstanding Report</b></h3><b></b></td>
	</tr>
	<tr>
...
$toDate=$_GET['todate'];
	...
	if(isset($_GET['fromdate'],$_GET['todate'])){
		$item_where= " bill_date BETWEEN '$fromDate' AND '$toDate'";
	}
	if(isset($_GET['val'])){
...
if(isset($_GET['comp'])){	
	$sql=mysql_query("select * from ar_bills where $item_where ");
...
$sql=mysql_query("select * from ar_bills");	
	...
	<tr>
		...
		<td width="80" class="fstChUPPRCase"><?php echo $row['bill_amount']; ?></td>
		<td width="80" class="fstChUPPRCase"><?php echo $remarks;?></td>
	</tr>
	<?php } } else { ?>	
	<div style="margin: 21px 0 26px 10px;;width:95%;" class="alert alert-success">
                               You have not created any Outstanding report details...
    </div>
<?php } ?>
</table>
	</div>
