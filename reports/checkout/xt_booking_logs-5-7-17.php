...
$phone=$rowPd['phone'];
	...
	<tr>
		<td colspan="18" style="text-align:center;font-size:14px;font-weight:bold;" >'.$prop_name.'</td>
	</tr>	
	<tr>
		<td colspan="18" style="text-align:center;font-size:14px;font-weight:bold;">Booking Status</td>
	</tr>
...
$item_where= " where str_to_date(book_date,'%d/%m/%Y') >= '$frm' AND str_to_date(book_date,'%d/%m/%Y') <= '$tod' AND confirm_status='".$_GET['sts']."' order by str_to_date(book_date,'%d/%m/%Y') DESC";
...
$item_where= " where str_to_date(book_date,'%d/%m/%Y') >= '$frm' AND str_to_date(book_date,'%d/%m/%Y') <= '$tod' order by str_to_date(book_date,'%d/%m/%Y') DESC";
$sql=mysql_query("select * from bq_hallbooking $item_where");
