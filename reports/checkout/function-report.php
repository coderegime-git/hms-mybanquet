...
$functParam  = isset($_GET['funct']) ? trim($_GET['funct']) : '';
...
if ($functParam != '' && $functParam != 'all') {
    $fEsc = mysql_real_escape_string($functParam);
    $where .= " AND funct = '$fEsc' ";
}
...
function getPageUrl($p, $frDateParam, $toDateParam, $functParam, $valParam, $limit) {
    return "function-report.php?fromdate=" . urlencode($frDateParam) . "&todate=" . urlencode($toDateParam) . "&funct=" . urlencode($functParam) . "&val=" . urlencode($valParam) . "&limit=" . $limit . "&page=" . $p;
}
?>
...
<style type="text/css">
/* ==========================================================================
   Master Pages Style - Standardized Unified Function Sales Report (MyPay)
   ========================================================================== */
body, body.bgBODY {
...
.mypay-table tbody tr.function-group-row td {
...
function clkSubmit() {
    ...
    document.location = "function-report.php?fromdate=" + encodeURIComponent(fromdate) + "&todate=" + encodeURIComponent(todate) + "&funct=" + encodeURIComponent(funct) + "&val=" + encodeURIComponent(srtx) + "&limit=" + encodeURIComponent(limit) + "&page=1";
}
...
function changeLimit(lim) {
    ...
    var funct = $('#funct').val() || '';
    var srtx = $('#searchTxt').val().trim();
    document.location = "function-report.php?fromdate=" + encodeURIComponent(fromdate) + "&todate=" + encodeURIComponent(todate) + "&funct=" + encodeURIComponent(funct) + "&val=" + encodeURIComponent(srtx) + "&limit=" + encodeURIComponent(lim) + "&page=1";
}

function srcSub(){
    document.location = "function-report.php";
}
...
function printPage(){
    ...
    printWindow.document.write('<html><head><title>Function Sales Report</title>');
    printWindow.document.write('<style>table {width:100%;border-collapse:collapse;font-family:Arial,sans-serif;font-size:11px;} th, td {border:1px solid #ccc;padding:5px;text-align:center;} th {background-color:#0073B5;color:#fff;} .text-right {text-align:right;} .text-left {text-align:left;} th.banner-row th {font-size:13px;} .function-group-row td {background-color:#e8f4fd !important;color:#005b8a !important;font-weight:bold;text-align:left;}</style>');
    ...
    printWindow.document.write('<h4 style="text-align:center;font-family:Arial;margin-top:0;margin-bottom:10px;">FUNCTION SALES REPORT (<?php echo htmlspecialchars($displayFrom); ?> to <?php echo htmlspecialchars($displayTo); ?>)</h4>');
...
<div class="mypay-container">
    ...
    <div class="mypay-actions-bar">
        <div class="mypay-filter-group">
            ...
            <span class="mypay-filter-label" style="margin-left:4px;">Function:</span>
            <select name="funct" id="funct" class="mypay-select">
                <option value="">All Functions</option>
                <?php 
                $sqlBS = mysql_query("select distinct func_code, func_desc from bq_function where status='1' order by func_desc ASC");
                if ($sqlBS) {
                    while ($rowBS = mysql_fetch_array($sqlBS)) {
                        $sel = ($rowBS['func_code'] == $functParam) ? 'selected' : '';
                        echo '<option value="' . htmlspecialchars($rowBS['func_code']) . '" ' . $sel . '>' . htmlspecialchars($rowBS['func_desc']) . '</option>';
                    }
                }
                ?>
            </select>
            ...
            <?php if(($valParam != '') || ($frDateParam != '') || ($functParam != '')) { ?>
                <button type="button" class="btn-mypay-reset" onclick="srcSub()" title="Clear Filter">
                    <i class="fa fa-times"></i> <span>Clear</span>
                </button>
            <?php } ?>

            <a href="<?php echo $home_path ?>/reports/checkout/xt_funcsales_report_xls.php?fromdate=<?php echo urlencode($displayFrom); ?>&todate=<?php echo urlencode($displayTo); ?>&funct=<?php echo urlencode($functParam); ?>" class="btn-mypay-excel" title="Export to Excel">
    ...
    <div class="mypay-table-wrapper" id="dvContainer">
        <table class="mypay-table" cellpadding="0" cellspacing="0">
            <thead>
                <tr class="banner-row">
                    <th colspan="<?php echo $colSpanTotal; ?>">FUNCTION SALES REPORT</th>
                </tr>
                <tr class="header-row">
            ...
            <?php 
            // Paginated query
            $sql = mysql_query("select * from bq_opbillhdr $where order by funct ASC, opbillhdr_id ASC LIMIT $offset, $limit");
            $x = $offset;
            ...
            if ($sql && is_resource($sql) && mysql_num_rows($sql) > 0) {
                while ($row = mysql_fetch_array($sql)) {
                    ...
                    $funcCodeCurrent = !empty($row['funct']) ? $row['funct'] : $rbk['funct'];
                    $rbf = mysql_fetch_array(mysql_query("select func_desc from bq_function where func_code='".$funcCodeCurrent."'"));
                    $funcDesc = !empty($rbf['func_desc']) ? $rbf['func_desc'] : 'GENERAL';

                    // If function changed, display a clean function subheader row
                    if ($funcCodeCurrent !== $currentFuncGroup && empty($functParam)) {
                        $currentFuncGroup = $funcCodeCurrent;
                    ?>
                        <tr class="function-group-row">
                            <td colspan="<?php echo $colSpanTotal; ?>">
                                <i class="fa fa-folder-open-o"></i> FUNCTION: <?php echo htmlspecialchars(strtoupper($funcDesc)); ?>
                            </td>
                        </tr>
                    <?php
                    }
            ...
            ?>
                <tr class="<?php echo $rowClass; ?>">
                    ...
                    <td class="text-left"><?php echo htmlspecialchars(strtoupper($funcDesc)); ?></td>
            ...
            ?>
                <tr>
                    <td colspan="<?php echo $colSpanTotal; ?>" style="padding: 20px 10px !important; color: #777777; text-align: center; font-size: 13px;">
                        No Function Sales Report records found for the selected criteria
                    </td>
                </tr>
            <?php } ?>
            </tbody>
    ...
    <div class="mypay-pagination-bar">
        ...
        <div class="mypay-pagination">
            <!-- First & Prev -->
            <?php if ($page > 1) { ?>
                <a href="<?php echo getPageUrl(1, $frDateParam, $toDateParam, $functParam, $valParam, $limit); ?>" class="mypay-page-link" title="First Page">&laquo;</a>
                <a href="<?php echo getPageUrl($page - 1, $frDateParam, $toDateParam, $functParam, $valParam, $limit); ?>" class="mypay-page-link" title="Previous Page">&lsaquo;</a>
            <?php } else { ?>
            ...
            if ($startPage > 1) {
                echo '<a href="' . getPageUrl(1, $frDateParam, $toDateParam, $functParam, $valParam, $limit) . '" class="mypay-page-link">1</a>';
                if ($startPage > 2) {
            ...
            for ($p = $startPage; $p <= $endPage; $p++) {
                if ($p == $page) {
                    ...
                    echo '<a href="' . getPageUrl($p, $frDateParam, $toDateParam, $functParam, $valParam, $limit) . '" class="mypay-page-link">' . $p . '</a>';
                }
            }
            ...
            if ($endPage < $totalPages) {
                ...
                echo '<a href="' . getPageUrl($totalPages, $frDateParam, $toDateParam, $functParam, $valParam, $limit) . '" class="mypay-page-link">' . $totalPages . '</a>';
            }
            ?>

            <!-- Next & Last -->
            <?php if ($page < $totalPages) { ?>
                <a href="<?php echo getPageUrl($page + 1, $frDateParam, $toDateParam, $functParam, $valParam, $limit); ?>" class="mypay-page-link" title="Next Page">&rsaquo;</a>
                <a href="<?php echo getPageUrl($totalPages, $frDateParam, $toDateParam, $functParam, $valParam, $limit); ?>" class="mypay-page-link" title="Last Page">&raquo;</a>
            <?php } else { ?>
