...
$(document).ready(function(){
 ...
 shortcut.add("Ctrl+S",function() { 
	 $('#taxTypes').attr('action', '../../action/add_subMnuGrpCode_bqt.php');  
	 $('#taxTypes').submit(); 
}); 
...
.spanClr{
	...
	<h3 id="Userhd"><b>Sub Menu Group Code(BQT)</b></h3>
		<form id="taxTypes" name="taxTypes" enctype="multipart/form-data"  action="<?php echo $home_path;?>/action/add_subMnuGrpCode_bqt.php" method="post" class="" style="">
<div>
...
<tbody>
	<tr>
		...
		<td valign="top"><input type="text" name="submenu_code" id="submenu_code" data-validation="required" class="textbox input validate[required] codesUPPERCase tobCode" onblur="checkDepartmentCode();" /></td>
	</tr>
	<tr>
		<td width="" valign="top"><label>SubMenu Group Name<em>*</em></label></td>
		<td valign="top"><input type="text" name="submenu_name" id="submenu_name" data-validation="required" class="textbox input validate[required] fstChUPPRCase" onblur="checkDepartmentName();"/></td>
		
	</tr>
<tr>
<td width="" valign="top"><label>Main Menu Group<em>*</em></label></td>
<td valign="top">
<select name="subgrp_code" id="subgrp_code" style="font-size:12px;" data-validation="required" onChange="selTaxCode(<?php echo $c;?>);" class="input validate[required] wagRw1 textbox ">
<option value="">--Select--</option>
<?php
$sqle=mysql_query("select * from bq_menugrp");
while($res=mysql_fetch_array($sqle)){
?>
<option value="<?php echo $res['menu_name'] ?>" ><?php echo $res['menu_name']; ?></option>
<?php } ?>
</select>
