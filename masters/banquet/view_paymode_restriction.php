...
<div class="mypay-container">
    ...
    <div class="mypay-actions-bar">
        <a href="paymode_restriction.php" class="btn-mypay-add" id="add" title="Add Paymode Restriction (Ctrl+A)">
            <span class="mypay-icon-plus"><i class="fa fa-plus"></i></span>
            <span>Add Paymode Restriction</span>
        </a>
    ...
    <table class="mypay-table" cellpadding="0" cellspacing="0">
        <thead>
            <tr class="banner-row">
                <th colspan="4">VIEW PAYMODE RESTRICTION DETAILS</th>
            </tr>
            <tr class="header-row">
                <th style="width: 25%;">Outlet Code</th>
        ...
        <tbody>
        <?php 
        $sql = mysql_query("select * from pos_paymode order by outlet_code, payment_type");
        if($sql && is_resource($sql) && mysql_num_rows($sql) > 0) {
            while($row = mysql_fetch_array($sql)) {
                $access = ucfirst($row['access']);
        ?>
            <tr>
                ...
                <td>
                    <a href="paymode_restriction.php?outlet=<?php echo urlencode($row['outlet_code']); ?>" class="mypay-edit-btn" title="Edit">
        ...
        ?>
            <tr>
                <td colspan="4" style="padding: 16px; color: #777; text-align: center; font-size: 13px;">
                    No Paymode Restriction records found
                </td>
            </tr>
        <?php } ?>
        </tbody>
