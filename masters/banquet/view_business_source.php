...
<div class="mypay-container">
    ...
    <div class="mypay-actions-bar">
        <a href="business-source-master.php" class="btn-mypay-add" id="add" title="Add Business Source (Ctrl+A)">
            <span class="mypay-icon-plus"><i class="fa fa-plus"></i></span>
            <span>Add Business Source</span>
        </a>
    ...
    <table class="mypay-table" cellpadding="0" cellspacing="0">
        <thead>
            <tr class="banner-row">
                <th colspan="4">VIEW BUSINESS SOURCE</th>
            </tr>
            <tr class="header-row">
        ...
        <tbody>
        <?php 
        $sql = mysql_query("select * from bq_bssource order by bssrc_id asc");
        if($sql && is_resource($sql) && mysql_num_rows($sql) > 0) {
            while($row = mysql_fetch_array($sql)) {
                $status = ($row['status'] == 1) ? "Active" : "Passive";
        ?>
            <tr>
                <td><?php echo htmlspecialchars($row['bs_code']); ?></td>
                <td><?php echo htmlspecialchars($row['bs_name']); ?></td>
                <td><?php echo $status; ?></td>
                <td>
                    <a href="edit_businesssource.php?bssrc_id=<?php echo $row['bssrc_id']; ?>" class="mypay-edit-btn" title="Edit">
        ...
        ?>
            <tr>
                <td colspan="4" style="padding: 16px; color: #777; text-align: center; font-size: 13px;">
                    No Business Source records found
                </td>
            </tr>
        <?php } ?>
        </tbody>
