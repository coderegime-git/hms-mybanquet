...
<script>
 ...
 shortcut.add("Ctrl+A",function() { 
 	 window.location.href = "business-source-master.php";
}); 
...
<div style="margin:0px 0 10px 750px;">
		<a href="business-source-master.php"><button type="button" id="add" class="button_example bnkSbt" onclick="return checkUnitMaster();" style="margin:4px 0 -8px 209px;"><img src="../../images/add-contact-iconn.png" class="sbtBtnImg"/>&nbsp;&nbsp;<span class="btnUndLine">A</span>dd Business source</button></a>
</div>
<table class="table table-condensed table-hover table-striped table-bordered" cellpadding="0" cellspacing="0" border="0" class="table" style="margin:0 0 15px -5px;text-align:center;font-size:12px;">
	<tr class="info">
		...
		<td colspan="13" style="text-align:center;"><h3 class="viewDT" id="Userhd"><b>View Business source</b></h3><b></b></td>
	</tr>
	<tr>
	...
	<?php 
	$sql=mysql_query("select * from bq_bssource");
	$x=0;
	...
	<tr>
		...
		<td width="80"><?php echo $row['bs_code']; ?></td>
		<td width="80" class="codesUPPERCase"><?php echo $row['bs_name']; ?></td>
		<td width="80" class="fstChUPPRCase"><?php echo $status; ?></td>
		<td width="80">
		<a href="edit_businesssource.php?bssrc_id=<?php echo $row['bssrc_id']; ?>" style="" class="">Edit</a>&nbsp;
		<input type="hidden" id="roomFeatid" name="roomid" value="<?php echo $row['bssrc_id']; ?>"/>
		</td>
	</tr>
	<?php } } else{ ?>	
