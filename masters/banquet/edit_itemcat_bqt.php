...
$itemcat_id = isset($_GET['itemcat_id']) ? mysql_real_escape_string($_GET['itemcat_id']) : '';
$sql = mysql_query("select * from bq_itemcat where itemcat_id='$itemcat_id'");
...
$(document).ready(function(){
    if(typeof jQuery.fn.validationEngine !== 'undefined') {
        $("#editItemCatForm").validationEngine();
    }
    $("#msgFo").fadeOut(5000);
    $("#cat_name").focus();
    ...
    if(typeof shortcut !== 'undefined') {
        shortcut.add("Ctrl+S", function() { 
            $('#editItemCatForm').attr('action', '../../action/update_itemcategory_bqt.php');  
            $('#editItemCatForm').submit(); 
        }); 
...
<div class="mypay-container">
    ...
    <!-- Edit Form Card Matching Market Segment Design Exactly -->
    <div class="mypay-card">
        <div class="mypay-card-header">
            UPDATE ITEM CATEGORY
        </div>

        <form id="editItemCatForm" name="editItemCatForm" enctype="multipart/form-data" action="<?php echo $home_path;?>/action/update_itemcategory_bqt.php" method="post">
            <input type="hidden" name="itemcat_id" id="itemcat_id" value="<?php echo htmlspecialchars($itemcat_id); ?>"/>
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td class="mypay-input-cell">
                                <input type="text" name="cat_code" id="cat_code" class="mypay-input" style="text-transform:uppercase;" value="<?php echo isset($row['cat_code']) ? htmlspecialchars($row['cat_code']) : ''; ?>" readonly />
                            </td>
                        </tr>
                        <tr>
                            <td class="mypay-label-cell">
                                <label for="cat_name">Category Name<span class="req-star">*</span> :</label>
                            </td>
                            <td class="mypay-input-cell">
                                <input type="text" name="cat_name" id="cat_name" data-validation-engine="validate[required]" class="mypay-input validate[required]" value="<?php echo isset($row['cat_name']) ? htmlspecialchars($row['cat_name']) : ''; ?>" required />
                            </td>
                        </tr>
                        <tr>
                            <td class="mypay-label-cell">
                                <label for="grp_name">Group Code<span class="req-star">*</span> :</label>
                            </td>
                            <td class="mypay-input-cell">
                                <select name="grp_name" id="grp_name" data-validation-engine="validate[required]" class="mypay-input validate[required]" required>
                                    <option value="">--Select Group--</option>
                                    <?php
                                    $sqle = mysql_query("select * from bq_grpcode order by grpcode asc");
                                    if($sqle && mysql_num_rows($sqle) > 0) {
                                        while($resulte = mysql_fetch_array($sqle)){
                                            $selected = (isset($row['grp_name']) && $resulte['grpcode'] == $row['grp_name']) ? 'selected' : '';
                                    ?>
                                        <option value="<?php echo htmlspecialchars($resulte['grpcode']); ?>" <?php echo $selected; ?>>
                                            <?php echo htmlspecialchars($resulte['grpname']); ?>
                                        </option>
                                    <?php 
                                        }
                                    }
            ...
            <div class="mypay-card-footer">
                ...
                <a href="view_categ_bqt.php" class="btn-mypay-action" title="View (Ctrl+V)">
