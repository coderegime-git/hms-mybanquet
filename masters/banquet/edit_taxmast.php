...
<?php 
	$sql=mysql_query("select * from bq_taxmast where taxmast_id='".$_GET['taxId']."'");
	$row=mysql_fetch_array($sql);
?>			
	<div id="addcustomer" class="frmCentr divBrd frmBgClr" style="width:468px;">
	<h3 id="Userhd"><b>Tax Master(BQT)</b></h3>
		<form id="taxTypes" name="taxTypes" enctype="multipart/form-data"  action="<?php echo $home_path;?>/action/update_tax_master.php" method="post" class="" style="">
		<input type="hidden" name="taxmast_id" id="taxmast_id" value="<?php echo $_GET['taxId'];?>"/>
		<div>
			...
			<tbody>
					<tr>
						...
						<td valign="top"><input type="text" name="tax_code" id="tax_code" data-validation="required" class="textbox input validate[required] codesUPPERCase tobCode" value="<?php echo $row['tax_code'];?>" readonly onblur="checkBusinessSource();" /></td>
					</tr>
					<tr>
						<td width="" valign="top"><label>Tax Name<em>*</em></label></td>
						<td valign="top"><input type="text" name="tax_desc" id="tax_desc" data-validation="required" class="textbox input validate[required] fstChUPPRCase" value="<?php echo $row['tax_desc'];?>" onblur="checkBusinessDesc();"/></td>
