...
jQuery(document).ready(function(){
    ...
    if(typeof shortcut !== 'undefined') {
        shortcut.add("Ctrl+A", function() { 
            window.location.href = "location_master_bqt.php";
        }); 
...
<div class="mypay-container">
    ...
    <div class="mypay-actions-bar">
        <a href="location_master_bqt.php" class="btn-mypay-add" id="add" title="Add Location (Ctrl+A)">
    ...
    <table class="mypay-table" cellpadding="0" cellspacing="0">
        <thead>
            <tr class="banner-row">
                <th colspan="4">VIEW LOCATION DETAILS</th>
            </tr>
            <tr class="header-row">
        ...
        <tbody>
        <?php 
        $sql = mysql_query("select * from bq_location order by locat_id asc");
        if($sql && is_resource($sql) && mysql_num_rows($sql) > 0) {
            while($row = mysql_fetch_array($sql)) {
                $status = ($row['status'] == 1) ? "Active" : "Passive";
        ?>
            <tr>
                <td><?php echo htmlspecialchars($row['loc_code']); ?></td>
                <td><?php echo htmlspecialchars($row['loc_desc']); ?></td>
                <td><?php echo $status; ?></td>
                <td>
                    <a href="edit_location.php?locMd=<?php echo $row['locat_id']; ?>" class="mypay-edit-btn" title="Edit">
