...
$(document).ready(function(){
 ...
 shortcut.add("Ctrl+S",function() { 
	 $('#hotelDefi').attr('action', '<?php echo $home_path;?>/action/update_bqtitem_master.php');  
	 $('#hotelDefi').submit(); 
}); 
...
<?php 
$sql=mysql_query("select * from bq_itemmaster where item_id='".$_GET['item_id']."'");
$row=mysql_fetch_array($sql);
?>				
	<div id="addcustomer" class="frmCentr divBrd frmBgClr" style="width:603px;">
	<h3 id="Userhd"><b>Update Item Master(BQT)</b></h3>
		<form id="hotelDefi" name="hotelDefi" enctype="multipart/form-data"  action="<?php echo $home_path;?>/action/update_bqtitem_master.php" method="post" class="" style="">
		<div>
		...
		<input type="hidden" name="taxCodee" id="taxCodee" class="txCde"/>
			...
			<tbody>
				<tr>
						...
						<td valign="top"><input type="text" name="item_code" id="item_code" data-validation="required" class="textbox input validate[required] codesUPPERCase tobCode" onblur="checkDepartmentCode();" value="<?php echo $row['item_code'];?>" readonly /></td>
				</tr>
				<tr>
						<td width="" valign="top"><label>Item Name<em>*</em></label></td>
						<td valign="top"><input type="text" name="item_name" id="item_name" data-validation="required" class="textbox input validate[required] fstChUPPRCase" onblur="checkDepartmentName();" value="<?php echo $row['item_name'];?>"/></td>
				</tr>
				
				<tr>
				<td width="" valign="top"><label>Menu type<em>*</em></label></td>
				<td valign="top">
				...
				?>
				<?php if($res['grpcode']==$row['menu_type']){?>
				<option value="<?php echo $res['grpcode'] ?>" selected ><?php echo $res['grpname']; ?></option>
				<?php }else{ ?>
				...
				while($res=mysql_fetch_array($sqle)){ 
				?>
				<?php if($res['subcat_code']==$row['itmsub_cat']){?>
				<option value="<?php echo $res['subcat_code'] ?>" selected ><?php echo $res['subcat_code']; ?></option>
				<?php }else{ ?>
				...
				while($res=mysql_fetch_array($sqle)){
				?>
				<?php if($res['submenu_code']==$row['itmsubmnu_code']){?>
				<option value="<?php echo $res['submenu_code'] ?>" selected ><?php echo $res['submenu_name']; ?></option>
				<?php }else{ ?>
				...
				<?php } } ?>
					...
					<tr>
						...
						<td valign="top"><input type="text" name="item_rate" id="item_rate" data-validation="required" class="textbox input validate[required] fstChUPPRCase" onblur="checkDepartmentName();" value="<?php echo $row['item_rate'];?>"/></td>
			...
			<select name="tax_struc" id="tax_struc" style="font-size:12px;" onChange="selTaxCode(<?php echo $c;?>);" class="wagRw1 textbox ">
				...
				while($res=mysql_fetch_array($sqle)){
				?>
				<?php if($res['str_code']==$row['tax_struc']){?>
				<option value="<?php echo $res['str_code'] ?>" selected ><?php echo $res['str_code']; ?></option>
				<?php }else{ ?>
...
<tr>					
					...
					<td width="" valign="top">
						<select name="allow_disc" id="allow_disc" class="textbox">
							<option value="">--Select--</option>
							<option value="yes"<?php echo ($row['allow_disc']=='yes')?'selected':''; ?>>yes</option>
							<option value="no"<?php echo ($row['allow_disc']=='no')?'selected':''; ?>>No</option>
						</select>
					</td>
					</tr>
			...
			<tr>
						...
						<select name="allwrate_chg" id="allwrate_chg" class="textbox" >
							<option value="">--Select--</option>
							<option value="yes"<?php echo ($row['allwrate_chg']=='yes')?'selected':''; ?>>yes</option>
							<option value="no"<?php echo ($row['allwrate_chg']=='no')?'selected':''; ?>>No</option>
						</select>
						</td>
						</tr>
					</tr>
					...
					<select name="allow_qty" id="allow_qty" class="textbox">
						<option value="">--Select--</option>
					<option value="yes"<?php echo ($row['allow_qty']=='yes')?'selected':''; ?>>yes</option>
					<option value="no"<?php echo ($row['allow_qty']=='no')?'selected':''; ?>>No</option>
					</select>
					</td>
					</tr>					
					...
					<tr>
						...
						<td width="" valign="top"><input type="radio" name="status" id="status_active" value="1"<?php echo ($row['status']=='1')?'selected':''; ?>  class="textbox fstChUPPRCase" style="width:10px;margin:3px 0 0 0;" checked /><span class="spanClr">Active</span>
						<input name="status" id="status_passive" type="radio" value="0"<?php echo ($row['status']=='0')?'selected':''; ?> style="width:10px;margin:3px 0 0 0;float:left;"/><span class="spanClr">Passive</span></td>
...
while($rwM=mysql_fetch_array($sqlM)){
	$sqlI=mysql_query("select item_code from bq_itemmaster where item_id='".$_GET['item_id']."'");
	$rowI=mysql_fetch_array($sqlI);
	$item_code=$rowI['item_code'];
	
	$sqlt=mysql_query("select item_code,mnu_sts,itmnu_code,item_id from bq_itemmaster where item_code='".$rowI['item_code']."' AND itmnu_code='".$rwM['menu_code']."' AND status ='1'");
	$rowt=mysql_fetch_array($sqlt);
?>
<tr>
<input type="hidden" name="item_id[]" id="item_id" class="textbox input fstChUPPRCase" value="<?php echo $rowt['item_id'] ?>" readonly />
	...
	<option value="">--select--</option>
	<option value="yes"<?php echo ($rowt['mnu_sts']=='yes')?'selected':''; ?>>Yes</option>
	<option value="no"<?php echo ($rowt['mnu_sts']=='no')?'selected':''; ?>>No</option>
	</select>
	</td>
</tr>
<?php } } ?>
