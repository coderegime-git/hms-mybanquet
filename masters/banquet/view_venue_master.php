...
jQuery(document).ready(function(){
    ...
    if(typeof shortcut !== 'undefined') {
        shortcut.add("Ctrl+A", function() { 
            window.location.href = "venue_master_bqt.php";
        }); 
...
<div class="mypay-container">
    ...
    <div class="mypay-actions-bar">
        <a href="venue_master_bqt.php" class="btn-mypay-add" id="add" title="Add Venue (Ctrl+A)">
    ...
    <table class="mypay-table" cellpadding="0" cellspacing="0">
        <thead>
            <tr class="banner-row">
                <th colspan="9">VIEW VENUE DETAILS</th>
            </tr>
            <tr class="header-row">
        ...
        <tbody>
        <?php 
        $sql = mysql_query("select * from bq_venue order by venue_id asc");
        if($sql && is_resource($sql) && mysql_num_rows($sql) > 0) {
            while($row = mysql_fetch_array($sql)) {
                $status = ($row['status'] == 1) ? "Active" : "Passive";
        ?>
            <tr>
                <td><?php echo htmlspecialchars($row['venue_code']); ?></td>
                <td><?php echo htmlspecialchars($row['venue_desc']); ?></td>
                <td><?php echo htmlspecialchars($row['location']); ?></td>
                <td><?php echo htmlspecialchars($row['length']); ?></td>
                <td><?php echo htmlspecialchars($row['breadth']); ?></td>
                <td><?php echo htmlspecialchars($row['height']); ?></td>
                <td><?php echo htmlspecialchars($row['area']); ?></td>
                <td><?php echo $status; ?></td>
                <td>
                    <a href="edit_venue.php?venId=<?php echo $row['venue_id']; ?>" class="mypay-edit-btn" title="Edit">
