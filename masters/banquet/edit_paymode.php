...
$(document).ready(function(){
 ...
 shortcut.add("Ctrl+S",function() { 
	 $('#taxTypes').attr('action', '../../action/add_paymode_bqt.php');  
	 $('#taxTypes').submit(); 
}); 
 shortcut.add("Ctrl+V",function() { 
	window.location.href = "view_paymode_bqt.php";
});
...
function checkPaymodeDesc(){
	source_desc=$('#source_desc').val();
	$.ajax({
		type:'GET',
		url:'  ../../action/repeatPaymodeDesc.php',
			data:{
...
<?php 
	$sql=mysql_query("select * from bq_paymode where bqpay_id='".$_GET['payMd']."'");
	$row=mysql_fetch_array($sql);
?>			
	<div id="addcustomer" class="frmCentr divBrd frmBgClr" style="width:468px;">
	<h3 id="Userhd"><b>Paymode(BQT)</b></h3>
		<form id="taxTypes" name="taxTypes" enctype="multipart/form-data"  action="<?php echo $home_path;?>/action/update_paymode_bqt.php" method="post" class="" style="">
		<input type="hidden" name="pay_id" id="pay_id" value="<?php echo $_GET['payMd'];?>"/>
		<div>
			<table cellpadding="0" cellspacing="0" class="table" border="0" style="margin:4px 0 0 0;" >
			<tbody>
					<tr>
						<td width="" valign="top"><label>Payment Code <em>*</em></label></td>
						<td valign="top"><input type="text" name="pay_code" id="pay_code" data-validation="required" class="textbox input validate[required] codesUPPERCase tobCode" value="<?php echo $row['pay_code'];?>" onblur="checkBusinessSource();" readonly /></td>
					</tr>
					<tr>
						<td width="" valign="top"><label>Payment Description<em>*</em></label></td>
						<td valign="top"><input type="text" name="pay_desc" id="pay_desc" data-validation="required" class="textbox input validate[required] fstChUPPRCase" value="<?php echo $row['pay_desc'];?>" onblur="checkBusinessDesc();"/></td>
	...
	<div style="margin:0 0 0 0px;">
		...
		<a href="view_paymode.php"><button type="button" id="update" class="buttExam_sngl bnkSbt" onclick="return checkPropertyMasterq();"><img src="<?php echo $home_path; ?>/images/audit.png" class="sbtBtnImg "/>&nbsp;&nbsp;<span class="btnUndLine">V</span>iew</button></a>
