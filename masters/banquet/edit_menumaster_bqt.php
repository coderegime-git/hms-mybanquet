...
.buttExam_sngl {
	padding: 4px 20px;
		...
		<?php
$sqlC=mysql_query("select * from bq_menumaster where menu_code='".$_GET['mencde']."'");
$row=mysql_fetch_array($sqlC);
?>		
	<div id="addcustomer" class="frmCentr divBrd frmBgClr" style="width:412px;">
	<h3 id="Userhd"><b>Edit Menu Master(BQT)</b></h3>
<form id="hotelDefi" name="hotelDefi" enctype="multipart/form-data"  action="<?php echo $home_path;?>/action/update_menu_master.php" method="post" class="" style="">
		<div>
		...
		<input type="hidden" name="taxCodee" id="taxCodee" class="txCde"/>
			...
			<tbody>
				<tr>
						...
						<td valign="top"><input type="text" name="menu_code" id="menu_code" data-validation="required" class="input validate[required] textbox" value="<?php echo $row['menu_code'];?>" />
						</td>
				</tr>
				<tr>
					<td width="" valign="top"><label>Menu Name<em>*</em></label></td>
					<td valign="top"><input name="menu_name" id="menu_name" type="text" data-validation="required" class="input validate[required] textbox fstChUPPRCase" value="<?php echo $row['menu_name'];?>" /></td>
				</tr>
...
<?php
$sqC=mysql_query("select * from bq_menumaster where menu_code='".$_GET['mencde']."'");
$c=0;
$nmRs=mysql_num_rows($sqC);
while($row=mysql_fetch_array($sqC)){
	$c++;
?>	
<tr>	
	<input name="menmas_id[]" id="menmas_id<?php echo $c;?>" type="hidden" class="textbox fstChUPPRCase" style="width:150px;" value="<?php echo $row['menmas_id'];?>" />
...
if($resulte['menu_code']==$row['menu_group']){
?>
<option value="<?php echo $resulte['menu_code'] ?>" selected ><?php echo $resulte['menu_name']; ?></option>
<?php } else { ?>menu_name
<option value="<?php echo $resulte['menu_code'] ?>" ><?php echo $resulte['menu_name']; ?></option>
<?php } } ?>
</select>
</td>
<td style="width:150px;text-align:center;">
...
<option value="">--Select--</option>
<?php
 $sqle="select distinct submenu_code,submenu_name from bq_submenugrp";
 $rowe=mysql_query($sqle);while($resulte=mysql_fetch_array($rowe)){
if($resulte['submenu_code']==$row['submenu']){
?>
<option value="<?php echo $resulte['submenu_code'] ?>" selected ><?php echo $resulte['submenu_name']; ?></option>
<?php } else { ?>
<option value="<?php echo $resulte['submenu_code'] ?>" ><?php echo $resulte['submenu_name']; ?></option>
<?php } } ?>
</select>
</td>
<td style="text-align:center;"><input name="allow_qty[]" id="allow_qty" type="text" class="textbox fstChUPPRCase" style="width:100px;" value="<?php echo $row['allow_qty'];?>" /></td>
</tr>
<?php } ?>
<?php for($c=$nmRs+1;$c<15;$c++){ ?>
<tr>	
<td style="width:150px;text-align:center;">
<select name="menu_group[]" id="menu_group<?php echo $c;?>" style="font-size:12px;width:140px;height:18px;" onChange="selMenuGrp(<?php echo $c;?>);" class="wagRw1 fstChUPPRCase">
<option value="">--Select--</option>
...
<?php
 ...
 $rowe=mysql_query($sqle);while($resulte=mysql_fetch_array($rowe)){
?>
<option value="<?php echo $resulte['submenu_code'] ?>" ><?php echo $resulte['submenu_name']; ?></option><?php }?>
</select>
</td>
...
<div style="margin:0px 0 0 0px;">
	...
	<a href="<?php echo $home_path; ?>/masters/banquet/view-menu-master.php"><button type="button" id="exit" name="exit" class="buttExam_sngl" style=""><img src="../../images/exitBut.png" class="sbtBtnImg" />&nbsp;&nbsp;<span class="btnUndLine">E</span>xit</button></a>
