...
jQuery(document).ready(function(){
    ...
    if(typeof shortcut !== 'undefined') {
        shortcut.add("Ctrl+A", function() { 
            window.location.href = "seating_master_bqt.php";
        }); 
...
<div class="mypay-container">
    ...
    <div class="mypay-actions-bar">
        <a href="seating_master_bqt.php" class="btn-mypay-add" id="add" title="Add Seating (Ctrl+A)">
    ...
    <table class="mypay-table" cellpadding="0" cellspacing="0">
        <thead>
            <tr class="banner-row">
                <th colspan="4">VIEW SEATING DETAILS</th>
            </tr>
            <tr class="header-row">
        ...
        <tbody>
        <?php 
        $sql = mysql_query("select * from bq_seating order by seat_id asc");
        if($sql && is_resource($sql) && mysql_num_rows($sql) > 0) {
            while($row = mysql_fetch_array($sql)) {
                $status = ($row['status'] == 1) ? "Active" : "Passive";
        ?>
            <tr>
                <td><?php echo htmlspecialchars($row['seat_code']); ?></td>
                <td><?php echo htmlspecialchars($row['seat_desc']); ?></td>
                <td><?php echo $status; ?></td>
                <td>
                    <a href="edit_seating.php?SeaID=<?php echo $row['seat_id']; ?>" class="mypay-edit-btn" title="Edit">
