...
$(document).ready(function(){
    ...
    if(typeof shortcut !== 'undefined') {
        shortcut.add("Ctrl+S", function() { 
            $('#menuGrpForm').attr('action', '../../action/add_menugrp_bqt.php');  
            $('#menuGrpForm').submit(); 
        }); 
...
function checkMenuGrpCode() {
    var menu_code = $('#menu_code').val().trim();
    if(menu_code == '') return;
    $.ajax({
        type: 'GET',
        url: '../../action/repeatBqtMenuGrpCode.php',
        data: { menu_code: menu_code },
        success: function(data){
            if(data == 1){
                alert('Menu Group code already exists!');
                $('#menu_code').val('').focus();
            }
        }
    });
...
function checkMenuGrpName(){
    var menu_name = $('#menu_name').val().trim();
    if(menu_name == '') return;
    $.ajax({
        type: 'GET',
        url: '../../action/repeatBqtMenuGrpName.php',
        data: { menu_name: menu_name },
        success: function(data){
            if(data == 1){
                alert('Menu Group name already exists!');
                $('#menu_name').val('').focus();
            }
        }
    });
...
<div class="mypay-container">
    ...
    <div class="mypay-card">
        ...
        <form id="menuGrpForm" name="menuGrpForm" enctype="multipart/form-data" action="<?php echo $home_path;?>/action/add_menugrp_bqt.php" method="post">
            <div class="mypay-card-body">
                <table class="mypay-form-table" cellpadding="0" cellspacing="0">
                    <tbody>
                        <tr>
                            <td class="mypay-label-cell">
                                <label for="menu_code">Menu Group Code<span class="req-star">*</span> :</label>
                            </td>
                            <td class="mypay-input-cell">
                                <input type="text" name="menu_code" id="menu_code" data-validation-engine="validate[required]" class="mypay-input validate[required]" style="text-transform:uppercase;" onblur="checkMenuGrpCode();" required />
                            </td>
                        </tr>
                        <tr>
                            <td class="mypay-label-cell">
                            ...
                            <td class="mypay-input-cell">
                                <input type="text" name="menu_name" id="menu_name" data-validation-engine="validate[required]" class="mypay-input validate[required]" onblur="checkMenuGrpName();" required />
                            </td>
                        </tr>
                        <tr>
                            <td class="mypay-label-cell">
                            ...
                            <td class="mypay-input-cell">
                                <input type="text" name="disp_order" id="disp_order" data-validation-engine="validate[required]" class="mypay-input validate[required]" required />
                            </td>
                        </tr>
                        <tr>
                            <td class="mypay-label-cell">
