...
.spanClr{
	...
	<h3 id="Userhd"><b>Item Category(BQT)</b></h3>
		<form id="taxTypes" name="taxTypes" enctype="multipart/form-data"  action="<?php echo $home_path;?>/action/add_itemcategory_bqt.php" method="post" class="" style="">
		<div>
			...
			<tbody>
					<tr>
						...
						<td valign="top"><input type="text" name="cat_code" id="cat_code" data-validation="required" class="textbox input validate[required] codesUPPERCase tobCode" onblur="checkDepartmentCode();" /></td>
					</tr>
					<tr>
						<td width="" valign="top"><label>Category Name<em>*</em></label></td>
						<td valign="top"><input type="text" name="cat_name" id="cat_name" data-validation="required" class="textbox input validate[required] fstChUPPRCase" onblur="checkDepartmentName();"/></td>
						
					</tr>
<tr>
	<td width="" valign="top"><label>Group Code<em>*</em></label></td>
	<td valign="top">
		<select name="grp_name" id="grp_name" style="font-size:12px;height:18px;" data-validation="required" class="textbox input validate[required] fstChUPPRCase">
		<option value="">--Select--</option>
		<?php
		$sqle="select * from bq_grpcode";
		$rowe=mysql_query($sqle);while($resulte=mysql_fetch_array($rowe)){
		?>
		<option value="<?php echo $resulte['grpcode'] ?>" ><?php echo $resulte['grpname']; ?></option><?php }?>
		</select>
	</td>
</tr>
					<tr>
