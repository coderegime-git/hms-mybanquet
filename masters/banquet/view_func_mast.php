...
jQuery(document).ready(function(){
    ...
    if(typeof shortcut !== 'undefined') {
        shortcut.add("Ctrl+A", function() { 
            window.location.href = "function_master_bqt.php";
        }); 
...
<div class="mypay-container">
    ...
    <div class="mypay-actions-bar">
        <a href="function_master_bqt.php" class="btn-mypay-add" id="add" title="Add Function Master (Ctrl+A)">
            <span class="mypay-icon-plus"><i class="fa fa-plus"></i></span>
            <span>Add Function Master</span>
        </a>
    ...
    <table class="mypay-table" cellpadding="0" cellspacing="0">
        <thead>
            <tr class="banner-row">
                <th colspan="4">VIEW FUNCTION DETAILS</th>
            </tr>
            <tr class="header-row">
        ...
        <tbody>
        <?php 
        $sql = mysql_query("select * from bq_function order by func_id asc");
        if($sql && is_resource($sql) && mysql_num_rows($sql) > 0) {
            while($row = mysql_fetch_array($sql)) {
                $status = ($row['status'] == 1) ? "Active" : "Passive";
        ?>
            <tr>
                <td><?php echo htmlspecialchars($row['func_code']); ?></td>
                <td><?php echo htmlspecialchars($row['func_desc']); ?></td>
                <td><?php echo $status; ?></td>
                <td>
                    <a href="edit_funct.php?funMd=<?php echo $row['func_id']; ?>" class="mypay-edit-btn" title="Edit">
