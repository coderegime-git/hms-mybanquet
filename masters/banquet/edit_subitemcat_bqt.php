...
<?php 
	$sql=mysql_query("select * from bq_subcatitem where subcat_id='".$_GET['subcat_id']."'");
	$row=mysql_fetch_array($sql);
?>			
	<div id="addcustomer" class="frmCentr divBrd frmBgClr" style="width:468px;">
	<h3 id="Userhd"><b>Item Sub Category(BQT)</b></h3>
		<form id="taxTypes" name="taxTypes" enctype="multipart/form-data"  action="<?php echo $home_path;?>/action/update_itemSubcate_bqt.php" method="post" class="" style="">
		<input type="hidden" name="subcat_id" id="subcat_id" value="<?php echo $_GET['subcat_id'];?>"/>
		<div>
			...
			<tbody>
					<tr>
						...
						<td valign="top"><input type="text" name="subcat_code" id="subcat_code" data-validation="required" class="textbox input validate[required] codesUPPERCase tobCode" value="<?php echo $row['subcat_code'];?>" readonly onblur="checkDepartmentCode();" /></td>
					</tr>
					<tr>
						<td width="" valign="top"><label>SubCategory Name<em>*</em></label></td>
						<td valign="top"><input type="text" name="subcat_name" id="subcat_name" data-validation="required" class="textbox input validate[required] fstChUPPRCase" value="<?php echo $row['subcat_name'];?>" onblur="checkDepartmentName();"/></td>
...
<?php $sqle=mysql_query("select distinct cat_code,cat_name from bq_itemcat where status='1' AND cat_code!=''");
		?>
							<select name="cat_code" id="cat_code" data-validation="required" class="textbox input validate[required] fstChUPPRCase" style="font-size:12px;height:18px;">
							<option value="">--Select--</option>
							<?php
							while($resulte=mysql_fetch_array($sqle)){?>
								<?php if($resulte['cat_code']==$row['cat_code']) { ?>
							<option value="<?php echo $resulte['cat_code'];?>" selected ><?php echo $resulte['cat_name'];?></option>
							<?php } else { ?>
							<option value="<?php echo $resulte['cat_code'];?>" ><?php echo $resulte['cat_name'];?></option>
							<?php } } ?>
							</select>
