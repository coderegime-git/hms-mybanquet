...
$SeaID = isset($_GET['SeaID']) ? mysql_real_escape_string($_GET['SeaID']) : '';
$sql = mysql_query("select * from bq_seating where seat_id='$SeaID'");
...
.btn-mypay-action {
    ...
    box-sizing: border-box !important;
    display: inline-flex !items-center !important;
    justify-content: center !important;
    gap: 6px !important;
...
$(document).ready(function(){
    ...
    if(typeof shortcut !== 'undefined') {
        shortcut.add("Ctrl+S", function() { 
            $('#seatingForm').attr('action', '<?php echo $home_path;?>/action/update_seating_code.php');  
            $('#seatingForm').submit(); 
        }); 
...
<div class="mypay-container">
    ...
    <div class="mypay-card">
        <div class="mypay-card-header">
            UPDATE SEATING MASTER(BQT)
        </div>

        <form id="seatingForm" name="seatingForm" enctype="multipart/form-data" action="<?php echo $home_path;?>/action/update_seating_code.php" method="post">
            <input type="hidden" name="seat_id" id="seat_id" value="<?php echo htmlspecialchars($SeaID);?>"/>
                    ...
                    <tbody>
                        <tr>
                            <td class="mypay-label-cell">
                                <label for="seat_code">Seating Code<span class="req-star">*</span> :</label>
                            </td>
                            <td class="mypay-input-cell">
                                <input type="text" name="seat_code" id="seat_code" class="mypay-input" style="text-transform:uppercase;" value="<?php echo htmlspecialchars($row['seat_code']);?>" readonly />
                            </td>
                        </tr>
                        <tr>
                            <td class="mypay-label-cell">
                                <label for="seat_desc">Seating Name<span class="req-star">*</span> :</label>
                            </td>
                            <td class="mypay-input-cell">
                                <input type="text" name="seat_desc" id="seat_desc" data-validation-engine="validate[required]" class="mypay-input validate[required]" value="<?php echo htmlspecialchars($row['seat_desc']);?>" required />
                            </td>
                        </tr>
                        <tr>
                            <td class="mypay-label-cell">
            ...
            <div class="mypay-card-footer">
                ...
                <a href="view_seating.php" class="btn-mypay-action" title="View (Ctrl+V)">
