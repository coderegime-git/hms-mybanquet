...
$(document).ready(function(){
 ...
 shortcut.add("Ctrl+S",function() { 
	 $('#taxTypes').attr('action', '../../action/add_billing_instruction.php');  
	 $('#taxTypes').submit(); 
}); 
 shortcut.add("Ctrl+V",function() { 
	window.location.href = "view_billing_instruction.php";
});
...
function checkBillingInstruction() {
	source_code=$('#source_code').val();
	$.ajax({
		type:'GET',
		url:'../../action/repeatBillingInstruction.php',
			data:{
...
function checkBillingInstructionDesc(){
	source_desc=$('#source_desc').val();
	$.ajax({
		type:'GET',
		url:'  ../../action/repeaBillingInstructionDesc.php',
			data:{
...
<?php 
	$sql=mysql_query("select * from bq_billinstruc where billinst_id='".$_GET['blMd']."'");
	$row=mysql_fetch_array($sql);
?>			
	<div id="addcustomer" class="frmCentr divBrd frmBgClr" style="width:468px;">
	<h3 id="Userhd"><b>Billing Instruction(BQT)</b></h3>
		<form id="taxTypes" name="taxTypes" enctype="multipart/form-data"  action="<?php echo $home_path;?>/action/update_bill_instr.php" method="post" class="" style="">
		<input type="hidden" name="billinst_id" id="billinst_id" value="<?php echo $_GET['blMd'];?>"/>
		<div>
			<table cellpadding="0" cellspacing="0" class="table" border="0" style="margin:4px 0 0 0;" >
			<tbody>
					<tr>
						<td width="" valign="top"><label>Billing Instuction Code <em>*</em></label></td>
						<td valign="top"><input type="text" name="bill_code" id="bill_code" data-validation="required" class="textbox input validate[required] codesUPPERCase tobCode" value="<?php echo $row['bill_code'];?>" onblur="checkBusinessSource();" readonly /></td>
					</tr>
					<tr>
						<td width="" valign="top"><label>Billing Instruction Name<em>*</em></label></td>
						<td valign="top"><input type="text" name="bill_desc" id="bill_desc" data-validation="required" class="textbox input validate[required] fstChUPPRCase" value="<?php echo $row['bill_desc'];?>" onblur="checkBusinessDesc();"/></td>
					</tr>
					<tr>
	...
	<div style="margin:0 0 0 0px;">
		...
		<a href="view_bill_inst.php"><button type="button" id="update" class="buttExam_sngl bnkSbt" onclick="return checkPropertyMasterq();"><img src="<?php echo $home_path; ?>/images/audit.png" class="sbtBtnImg "/>&nbsp;&nbsp;<span class="btnUndLine">V</span>iew</button></a>
