...
<?php 
	$sql=mysql_query("select * from bq_submenugrp where submng_id='".$_GET['submng_id']."'");
	$row=mysql_fetch_array($sql);
?>				
	<div id="addcustomer" class="frmCentr divBrd frmBgClr" style="width:468px;">
	<h3 id="Userhd"><b>Sub Menu Group Code(BQT)</b></h3>
		<form id="taxTypes" name="taxTypes" enctype="multipart/form-data"  action="<?php echo $home_path;?>/action/update_subMnuGrpCode_bqt.php" method="post" class="" style="">
<input type="hidden" name="submng_id" id="submng_id" value="<?php echo $_GET['submng_id'];?>"/>
<div>
<table cellpadding="0" cellspacing="0" class="table" border="0" style="margin:4px 0 0 0;" >
<tbody>
	<tr>
		<td width="" valign="top"><label>SubMenu Group Code <em>*</em></label></td>
		<td valign="top"><input type="text" name="submenu_code" id="submenu_code" data-validation="required" class="textbox input validate[required] codesUPPERCase tobCode" value="<?php echo $row['submenu_code'];?>" readonly onblur="checkDepartmentCode();" /></td>
	</tr>
	<tr>
		<td width="" valign="top"><label>SubMenu Group Name<em>*</em></label></td>
		<td valign="top"><input type="text" name="submenu_name" id="submenu_name" data-validation="required" class="textbox input validate[required] fstChUPPRCase" value="<?php echo $row['submenu_name'];?>" onblur="checkDepartmentName();"/></td>
		
	</tr>
<tr>
<td width="" valign="top"><label>Main Menu Group<em>*</em></label></td>
<td valign="top">

<?php $sqle=mysql_query("select * from bq_menugrp");
		?>
							<select name="subgrp_code" id="subgrp_code" data-validation="required" class="textbox input validate[required] fstChUPPRCase" style="font-size:12px;height:18px;">
							<option value="">--Select--</option>
							<?php
							while($resulte=mysql_fetch_array($sqle)){?>
								<?php if($resulte['menu_code']==$row['subgrp_code']) { ?>
							<option value="<?php echo $resulte['menu_code'];?>" selected ><?php echo $resulte['menu_code'];?></option>
							<?php } else { ?>
							<option value="<?php echo $resulte['menu_code'];?>" ><?php echo $resulte['menu_code'];?></option>
							<?php } } ?>
							</select>
	...
	<div style="margin:0 0 0 0px;">
		...
		<a href="view_submenuBqt_bqt.php"><button type="button" id="update" class="buttExam_sngl bnkSbt" onclick="return checkPropertyMasterq();"><img src="<?php echo $home_path; ?>/images/audit.png" class="sbtBtnImg "/>&nbsp;&nbsp;<span class="btnUndLine">V</span>iew</button></a>
