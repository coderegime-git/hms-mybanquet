...
$(document).ready(function(){
 ...
 shortcut.add("Ctrl+S",function() { 
	 $('#taxTypes').attr('action', '../../action/add_function_master.php');  
	 $('#taxTypes').submit(); 
}); 
 shortcut.add("Ctrl+V",function() { 
	window.location.href = "view_function_master.php";
});
...
function checkFunctionMaster() {
	source_code=$('#source_code').val();
	$.ajax({
		type:'GET',
		url:'../../action/repeatFunctionMaster.php',
			data:{
...
function checkFunctionMasterDesc(){
	source_desc=$('#source_desc').val();
	$.ajax({
		type:'GET',
		url:'  ../../action/repeatFunctionMasterDesc.php',
			data:{
...
<?php 
	$sql=mysql_query("select * from bq_function where func_id='".$_GET['funMd']."'");
	$row=mysql_fetch_array($sql);
?>				
	<div id="addcustomer" class="frmCentr divBrd frmBgClr" style="width:468px;">
	<h3 id="Userhd"><b>Function Master(BQT)</b></h3>
		<form id="taxTypes" name="taxTypes" enctype="multipart/form-data"  action="<?php echo $home_path;?>/action/update_func_mast.php" method="post" class="" style="">
		<input type="hidden" name="func_id" id="func_id" value="<?php echo $_GET['funMd'];?>"/>
		<div>
			<table cellpadding="0" cellspacing="0" class="table" border="0" style="margin:4px 0 0 0;" >
			<tbody>
					<tr>
						<td width="" valign="top"><label>Function Code <em>*</em></label></td>
						<td valign="top"><input type="text" name="func_code" id="func_code" data-validation="required" class="textbox input validate[required] codesUPPERCase tobCode" value="<?php echo $row['func_code'];?>" onblur="checkBusinessSource();" readonly /></td>
					</tr>
					<tr>
						<td width="" valign="top"><label>Function Name<em>*</em></label></td>
						<td valign="top"><input type="text" name="func_desc" id="func_desc" data-validation="required" class="textbox input validate[required] fstChUPPRCase" value="<?php echo $row['func_desc'];?>" onblur="checkBusinessDesc();"/></td>
	...
	<div style="margin:0 0 0 0px;">
		...
		<a href="view_func_mast.php"><button type="button" id="update" class="buttExam_sngl bnkSbt" onclick="return checkPropertyMasterq();"><img src="<?php echo $home_path; ?>/images/audit.png" class="sbtBtnImg "/>&nbsp;&nbsp;<span class="btnUndLine">V</span>iew</button></a>
