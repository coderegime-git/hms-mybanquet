...
if(isset($_POST['act'])){
	if($_POST['act']=='approve'){
		foreach($_POST['chk'] as $chk){
			$sql="update vendor_allocation set status='Approved' where vendorallot_id=".$chk;
			mysql_query($sql);
...
<div class="about">
	...
	<a id="approve" style="display:none;" class="btnn"><button type="button" id="approve" style="color:#000;" class="submitbtnedit btnn appV">Approve</button></a>
...
?>
		<tbody>
			...
			<td><a rel="popover" data-placement="right" data-original-title="Vendor Name # <?php echo $row['vendor_name']; ?>" data-content="<div style='padding:10px 0 0 0'><span style='padding:10px 10px 0 0'>RFQ</span><?php echo $row['rfq_no'];?></div><div style='padding:10px 0 0 0'><span style='padding:10px 10px 0 0'>Vendor Price</span><?php echo $row['vendor_price'];?></div></a></td>
			...
			<td class="fstChUPPRCase"><?php echo $row['allot_qty']; ?></td>
...
<input type="hidden" id="st_<?php echo $row['vendorallot_id']?>" value="<?php echo ($row['status'] == 'Approved')?1:0;?>" />
	</td>		
<td>
