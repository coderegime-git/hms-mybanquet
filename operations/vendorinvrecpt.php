...
$(document).ready(function(){
	...
	$("#rfq_no").keyup(function(){
		$.ajax({
		type: "POST",
		url: "../action/selectVENDinvRCPT.php",
	...
	$("#qty_accepted").keyup(function(){
		...
		if(qtyA!='' && qtyR!='' && qtyRe!=''){
			...
			var totQ=parseFloat(qtyA+qtyR);
		}else if(qtyA!='' && qtyRe!=''){
			var totQ=parseFloat(qtyA+qtyRe);
		}else if(qtyA!=''){
			var totQ=parseFloat(qtyA);
		}
		alert(totQ);
		/* alert('rtw22'+totQ); */
		var BaQy=parseFloat(parseFloat(ordQ)-parseFloat(totQ));
		$("#bal_qty").val(BaQy);
	...
	$.ajax({
		...
		url:'  ../action/selVenDRfqNo.php',
			...
			success:function(data){
				  ...
				  $(".venInvDt").val(Qte[6]);
				  $("#order_qty").val(Qte[4]);
				  $("#vendor_name").html(Qte[7]);
				  /* if(data==1){
					  alert('Please check Vendor PO Approval status!.');
				  } */
			}
	});
}
...
<form id="quotationmaster" name="quotationmaster" action="<?php echo $home_path;?>/action/add_vendorInvoiceRcpt.php" method="post" class="" style="margin: 0 0 12px 0;">
<div>
		...
		<tr>
				...
				<td>
				<input name="rfq_no" id="rfq_no" type="text" class="textbox" onclick="selRFQnumber();" />
				<div id="suggesstion-box"></div>
				<!--<select name="rfq_no" id="rfq_no" style="width:179px;" data-validation="required" class="input validate[required] textbox" onchange="selRFQnumber();">
				<option value="">--Select--</option>
				</select>-->
				<!--<input name="rfq_no" id="rfq_no" type="text" data-validation="required" class="input validate[required] textbox" onkeyup="selRFQnumber();"/>-->
				</td>
		</tr>
		<tr>
				<td width="180" >PO No :</td>
				<td><input name="purorder_no" id="purorder_no" type="text" data-validation="required" class="input validate[required] textbox"/></td>
		</tr>
		<tr>
			...
			<option value="">--Select--</option>
			<?php
			/* $sqle="select distinct vendor_name from vendor_po";
			...
			<tr>
				...
				<input name="vendor_invno" id="vendor_invno" type="text" data-validation="required" class="input validate[required] textbox" value="" />
				</td>
			</tr>
			<tr>
					<td width="180" >Vendor Invoice Date :</td>
					<td >
					<input name="vendor_invdate" id="calendar" type="text" data-validation="required" class="input validate[required] textbox venInvDt" onblur="checkitemcode()"/>
					</td>
			</tr>
			<tr>
				<td width="180" >Ordered Qty:</td>
				<td><input name="order_qty" id="order_qty" type="text" class="textbox " value="" onblur="excTotQTY();"  /></td>
			</tr>
		<!--<tr>
				<td width="180" >Ordered Qty :</td>
				<td ><input name="order_qty" id="order_qty" type="text" data-validation="required" class="input validate[required] textbox" /></td>
			</tr>-->
			<tr>
				<td width="180" >Qty Accepted:</td>
				<td>
					<input name="qty_accepted" id="qty_accepted" type="text" data-validation="required" class="input validate[required] textbox" onblur="calcQtyACCe();" style="width:90px;"/><input name="bal_qty" id="bal_qty" type="text" class="textbox " value="" style="width:89px;" placeholder="Balance qty" readonly />
				</td>
			</tr>
			
			<tr id="qtyRew" style="display:none;">
				<td width="180" >Qty Rework:</td>
				<td><input name="qty_rework" id="qty_rework" type="text" value="" class="textbox" placeholder="0.00"/></td>
			</tr>
			<tr id="qtyRej" style="display:none;">
				<td width="180" >Qty Rejected:</td>
				<td>
					<input name="qty_reject" id="qty_reject" type="text" value="" class="textbox" onblur="qtyReject()" placeholder="0.00"/>
				</td>
			</tr>
			<tr id="qtyRejRw" style="display:none;">
				<td width="180" >Reason for Rework/Reject :</td>
				<td><input name="reason_rework" id="reason_rework" type="text" class="textbox"/></td>
			</tr>
		</table>
		</div>
		...
		<table style="width:50%;border-left:1px solid #ddd;" class="table">
			...
			<tr>
				...
				<td ><input name="rate" id="rate" type="text" data-validation="required" class="input validate[required,custom[number]] textbox fstChUPPRCase"/></td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td ><input name="amount" id="amount" type="text" data-validation="required" class="input validate[required,custom[number]] textbox" readonly /></td>
			</tr>
			<tr>
				<td width="180" >Amount Payable:</td>
				<td ><input name="amount_payable" id="amount_payable" type="text" data-validation="required" class="input validate[required,custom[number]] textbox" readonly /></td>
			</tr>
			<tr>
				<td width="180" >Vendor DC. No. :</td>
				<td ><input name="vendor_dlno" id="vendor_dlno" type="text" class="textbox codesUPPERCase"/></td>
			</tr>
			<tr>
				<td width="180" >Vendor DC. Date :</td>
				<td>
					<input name="vendor_dldate" id="calendar2" type="text" class="textbox codesUPPERCase vendInvdate"/>
				</td>
			</tr>
			<!--<tr>
				<td width="180" >Packing Type:</td>
				<td><select class="drop-down fstChUPPRCase" name="packing_type" id="packing_type">
						<option value="">--Select--</option>
						<option value="curton box">Curton Box</option>
