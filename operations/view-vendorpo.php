<?php
ob_start();
include("../includes/header.php");

if(isset($_POST['act'])){
	if($_POST['act']=='approve'){
		foreach($_POST['chk'] as $chk){
			$sql="update vendor_po set status='generated' where vendorpo_id=".$chk;
			mysql_query($sql);
			$message="Vendor Generated";
		}
	}
}
 ?>
<script type="text/javascript" src="<?php echo $home_path; ?>/js/bootstrap.min.js"></script>
 <script type="text/javascript">
$(document).ready(function(){
	$("[rel=tooltip]").tooltip();
	$("[rel=popover]").popover({trigger:'hover',html:true});
	
	$('.btnn').click(function (e) {
		if($(this).attr('id')=="approve"){
			/* if($("input:checked").length>1){ alert("Please select only one row"); return } */
			r=confirm("Do you want to approve");
			if(r==true){
				$('#act').val("approve");
				$('#thisform').submit();
			}else{
				
			}
			
		}
		
	});
	
	 $(':checkbox').click(function(e){
																
                                if($("input:checked").length>0){
                                   $('#edit').show();
                                   $('#pay').show();
                                   $('#pdf').show();
                                   $('#email').show();
                                   $('#approve').show();
                                }else{
                                   $('#edit').hide();
                                   $('#pay').hide();
                                   $('#pdf').hide();
                                   $('#email').hide();
                                   $('#approve').hide();
								}
                               
                            });
});


					function setPrint(id,val)
						{	
							if($("#"+id).is(":checked"))
							{  
								//$('#qtyDeliver').attr('disabled','disabled');
								
								$('.ckPrint').each(function(){
									a_id=this.id.split('_');
									if($(this).attr('id') != id)
									{
										$(this).attr("disabled",true);
										$("#ed"+a_id[1]).attr("style","display:none");
									}
								});
							}
							else
							{
								$('.ckPrint').each(function(){
									a_id=this.id.split('_');
									$(this).removeAttr("disabled");
									$("#ed"+a_id[1]).attr("style","display:inline");
								});
							}
						}
						
						function popup()
						{
							id=$('.ckPrint:checkbox:checked').val();
							newwindow=window.open('<?php echo $home_path;?>/views/crescentvendorpo_printout.php?uid='+id,'mywin','left=120,width=1000,height=700,');
							newwindow.focus();
						}
						/* function popupDC()
						{
							id=$('.ckPrint:checkbox:checked').val();
							newwindow=window.open('<?php echo $home_path;?>/operations/customerpo.php?uid='+id,'_blank');
							newwindow.focus(); 
						} */
						

						
</script>
 <body class="bgBODY">
 
 
<div style="margin:10px 0 10px -75px;"><a class="submitbtnAdd" href="vendorpo.php">Add Vendor PO</a>
		</div>				
					
<div class="about">
	<div class="container" style="width:1000px;/* margin:0; */padding:0;">
	
					<div style="height:33px;margin:0 0 0 22px;" id="toolbar">
                        <div class="btn-group">
						<a id="approve" style="display:none;" class="btnn"><button type="button" id="approve" style="color:#000;" class="submitbtnedit btnn appV">Generate</button></a>
                        <button type="button" id="pdf" style="display:none" class="submitbtnprint btnn" onclick="popup()">Print</button>
					<div >

					<input value="<?php echo $_POST['keyword']?>" name="keyword" type="text" class="input-medium search-query" style="float:left;margin-left:635px;margin-top:-21px;display:none"/>
					<button style="margin-top: -4.1%;height: 21px;font-size: 12px;padding-top: 1px;float: left;display:none" class="btnn">Go</button>

                            </div>
                         </div>
                    </div>
					
<form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post" id="thisform">
<input type="hidden" id="act" name="act" value="" />					
		<div class="col-md-12" style="overflow:auto;width:1000px;height:470px;" >
		<h3 style="text-align:center;width:100%;font-size:14px;padding:10px;background:#0080C0;color:#fff;margin:1px 0 0 0;text-transform:uppercase;"><b>View Vendor PO</b></h3>
<table class="table table-condensed table-hover table-striped table-bordered" cellpadding="0" cellspacing="0" border="0" class="table">
	<?php 
	$sql=mysql_query("select * from vendor_po where company_id='".$_SESSION['companyId']."'");
	$x=0;
	if(mysql_num_rows($sql)>0) {
		?>
		 	<thead>
				<th>S No.</th>
				<th></th>
				<th>Type PO</th>
				<th>RFQ No</th>
				<th>Date</th>
				<th>Qty</th>
				<th>Rate</th>
				<th>Req Delivery date</th>
				<th>Po no</th>
				<th>Part no</th>
				<th>Part name</th>
				<th>Total amount</th>
				<th>Status</th>
				<th style="text-align:center;">Edit</th>
				
				<!--<th style="text-align:center;">Edit</th>-->
			</thead>
       <?php
	while($row=mysql_fetch_array($sql)) {
		$x++;
		if($row['typo_po']=='purchase order'){
			$typo_po='PO';
		}else{
			$typo_po='JO';
		}
		
	?>
		<tbody>
			<tr >
			<td><?php echo $x; ?></td>
			<td><input name="chk[]"  type="checkbox" id="c_<?php echo $row['vendorpo_id']?>" class="ckPrint group1 check_" value="<?php echo $row['vendorpo_id']?>" onclick="setPrint(this.id,this.value);" /></td>
			
			<td><a rel="popover" data-placement="right" data-original-title="Vendor Name # <?php echo $row['vendor_name']; ?>" data-content="<div style='padding:10px 0 0 0'><span style='padding:10px 10px 0 0'>RFQ</span><?php echo $row['rfq_no'];?></div><div style='padding:10px 0 0 0'><span style='padding:10px 10px 0 0'>Del Date</span><?php echo $row['req_deldate'];?></div><div style='padding:10px 0 0 0'><span style='padding:10px 10px 0 0'>Status</span><?php echo $row['status'];?></div></a></td>
			
			
			<td class="codesUPPERCase"><?php echo $typo_po; ?></td>
			<td class="fstChUPPRCase"><?php  echo $row['rfq_no']; ?></td>
			<td class="fstChUPPRCase"><?php  echo $row['cur_date']; ?></td>
			<td class="fstChUPPRCase"><?php  echo $row['qty']; ?></td>
			<td class="fstChUPPRCase"><?php  echo $row['rate']; ?></td>
			<td class="fstChUPPRCase"><?php  echo $row['req_deldate']; ?></td>
			<td class="fstChUPPRCase"><?php  echo $row['po_no']; ?></td>
			<td class="fstChUPPRCase"><?php  echo $row['part_no']; ?></td>
			<td class="fstChUPPRCase"><?php  echo $row['part_name']; ?></td>
			<td class="fstChUPPRCase"><?php  echo $row['total_amount']; ?></td>
			<?php if($row['status']=='generated'){ ?>
			<td class="fstChUPPRCase" style="color:#1B7FAD;"><?php echo $row['status']; ?>
			<?php }else{ ?>
			<td class="fstChUPPRCase"><?php echo $row['status']; ?>
			<?php } ?>
			
			<input type="hidden" id="st_<?php echo $row['vendorpo_id']?>" value="<?php echo ($row['status'] == 'generated')?1:0;?>" />
			</td>
			<td>
				<div style="display:inline"; id="ed<?php echo $row['vendorpo_id'];?>">
					<a title="Edit Quotation Details" href="update-vendorpo.php?uid=<?php echo $row['vendorpo_id']?>" style="color:#005580;">Edit</a>
				</div>
			</td>
			
<!--<td>		
<a title="Edit Invoice" onclick="enable_cb(this)" class="findedit group1 submitbtnedit" href="edit-invoice.php?uid=<?php /* echo $row['quote_id'] */?>" style="">edit</a>	
</td>			
			<td><a title="Edit Property Details" href="update-prop-master.php?uid=<?php /* echo $row['propmaster_id'] */?>" style="color:#005580;">Edit</a></td>-->
			</tr>
		</tbody>
		 <?php  }  ?>
	</table>
<?php  } else { ?>

		  <div style="margin:10px 0 10px 0" class="alert alert-success">
			   You have not created any Vendor Purchase Order Details...
		 </div>

		<?php  }  ?>

	</form>

		  
		  
		  
		  
		  
		  
		</div>
		</div>
		</div>