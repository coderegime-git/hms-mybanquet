	 ...
	 $(':checkbox').click(function(e){
					...
					function setPrint(id,val)
						...
						function popup()
						{
							...
							newwindow=window.open('<?php echo $home_path;?>/views/invoice_printout.php?uid='+id,'mywin','left=120,width=1000,height=700,');
							newwindow.focus();
						}
						/* function popupDC()
						{
...
<div style="margin:10px 0 10px -75px;"><a class="submitbtnAdd" href="invoicepage.php">Add Invoice</a>
		</div>				
...
<div class="about">
	<div class="container" style="width:1000px;/* margin:0; */padding:0;">
		...
		<h3 style="text-align:center;width:100%;font-size:14px;padding:10px;background:#0080C0;color:#fff;margin:1px 0 0 0;text-transform:uppercase;"><b>View Invoice</b></h3>
<table class="table table-condensed table-hover table-striped table-bordered" cellpadding="0" cellspacing="0" border="0" class="table">
	<?php 
	$sql=mysql_query("select * from invoice where company_id='".$_SESSION['companyId']."'");
	$x=0;
	...
	?>
		<tbody>
			<tr >
			<td><?php echo $x; ?></td>
			<td><input name="chk[]"  type="checkbox" id="c_<?php echo $row['invoice_id']?>" class="ckPrint group1 check_" value="<?php echo $row['invoice_id']?>" onclick="setPrint(this.id,this.value);" style="width:15px;" /></td>
			<td class="fstChUPPRCase"><?php  echo $row['invoice_date']; ?></td>
			<td class="codesUPPERCase"><?php echo $row['invoice_no']; ?></td>
			...
			<td class="fstChUPPRCase"><?php  echo $row['total_itemval']; ?></td>
			<td class="fstChUPPRCase"><?php  echo $row['inv_subtotal']; ?></td>
			<td>
			<div style="display:inline"; id="ed<?php echo $row['invoice_id'];?>">
			<a title="Edit Packing Page" href="update-invoice.php?uid=<?php echo $row['invoice_no']?>" style="color:#005580;">Edit</a>
			</div>
			</td>
...
<?php  } else { ?>
		  ...
		  <div style="margin:10px 0 10px 0" class="alert alert-success">
			   You have not created any Invoice Details...
		 </div>
