...
for($cc=0;$cc<count($item_name);$cc++){
	if($_POST['item_name']!="")	{
		...
		$sqll=$sqll." where opvchrdtl_id='".$_POST['opvchrdtl_id'][$cc]."' AND bill_status!='3'";
		/* $sqll=$sqll." where item_name='".$_POST['item_name'][$cc]."' AND vouchrno='".$_POST['voucher_no']."' AND opvchrdtl_id='".$_POST['opvchrdtl_id']."' AND bill_status!='3'"; */
		/* echo  */
...
for($ce=0;$ce<count($_POST['item_code']);$ce++){
 $sqF=mysql_query("select * from bq_taxstruct where str_code='".$_POST['taxcde'][$ce]."' AND status='1'");
	while($roF=mysql_fetch_array($sqF)){
		...
		$sqll=$sqll." where item_name='".$_POST['item_name'][$ce]."' AND vouchrno='".$_POST['voucher_no']."' AND taxcode='".$roF['tax_code']."' AND bill_status!='3'";
		 /* echo $sqll; */ 
		$resultt=mysql_query($sqll);
	} 
}
...
for($cf=0;$cf<count($_POST['item_code']);$cf++){
		...
		$sqll=$sqll." where item_name='".$_POST['item_name'][$cf]."' AND vouchrno='".$_POST['voucher_no']."' AND bill_status!='3'";
		/* echo $sqll; */
		$resultt=mysql_query($sqll);
}
/* die(); */
