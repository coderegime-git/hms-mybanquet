...
$sb=mysql_query("select * from bq_hallbooking where venue='".$venu."' AND confirm_status!='7' AND str_to_date(book_date,'%d/%m/%Y') = '$bkm'");
...
if($rb['confirm_status']=='2'){
	...
	$sts='Blocked';
				...
				if($rb['confirm_status']=='6'){
				$msg='Already Booked!. '.'Bk# '.$rb['booking_no'].' Gst name: '.strtoupper($rb['guest_name']).' Status: '.$sts;
				echo '1'.','.$msg.','.$rb['from_time'].','.$rb['to_time'].','.$rb['confirm_status'];   
			   }else{
				$msg1='Already Booked!. '.'Bk# '.$rb['booking_no'].' Gst name: '.strtoupper($rb['guest_name']).' Status: '.$sts;
				echo '2'.','.$msg1;
			   }
