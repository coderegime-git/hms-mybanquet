...
/*$sqlb=mysql_query("select * from bq_hallbooking where booking_no='".$row['bkno']."' AND hallbook_id='".$row['hallbook_id']."' AND confirm_status='2' and fpno='".$fp_no."'");*/
$sqlb=mysql_query("select * from bq_hallbooking where booking_no='".$row['bkno']."'  AND confirm_status='2' and fpno='".$fp_no."'");
...
$sqbC=mysql_query("select * from bq_opfpmenuhdr where fpno='".$fp_no."' AND hallbook_id='".$hallbook_id."' AND bill_status='1'");
...
if($rwbC['hallchrg']>0){
	...
	$sqhall=mysql_query("select * from bq_grpcode where grpcode='hall' AND status ='1'");
    $rohall=mysql_fetch_array($sqhall);

$output.='<tr id="">

	<td style="text-align:center;" class="sourceonVAL"><input name="item_code[]" id="item_code1" type="text"  class="textbox fstChUPPRCase expet" style="width:100px;margin:5px 0 0 0px" value="Hall" readonly />
	<input name="sac[]" id="sac" type="hidden"  class="textbox fstChUPPRCase expet" style="width:100px;margin:5px 0 0 0px"  value="'.$rohall['hsn'].'" readonly /></td>
...
$sqrte=mysql_query("select * from bq_grpcode where grpcode='rate' AND status ='1'");
    $rorte=mysql_fetch_array($sqrte);
	$totVRt=$rowb['guaranted']*$rwbC['ratechrg'];
$output.='<tr id="">
	<td style="text-align:center;" class="sourceonVAL"><input name="item_code[]" id="item_code1" type="text"  class="textbox fstChUPPRCase expet" style="width:100px;margin:5px 0 0 0px" value="'.$rq['menu_code'].'" readonly /><input name="sac[]" id="sac" type="hidden"  class="textbox fstChUPPRCase expet" style="width:100px;margin:5px 0 0 0px"  value="'.$rorte['hsn'].'" readonly /></td>
...
/* open item remove in voucher and billing (17.04.26)  */
...
$totOpn+=$rbC['qty']*$rbC['rate'];
	...
	<td style="text-align:center;" class="sourceonVAL"><input name="item_code[]" id="item_code" type="text"  class="textbox fstChUPPRCase expet" style="width:100px;margin:5px 0 0 0px"  value="'.$rbC['itemcode'].'" readonly /><input name="sac[]" id="sac" type="hidden"  class="textbox fstChUPPRCase expet" style="width:100px;margin:5px 0 0 0px"  value="'.$rbC['sac'].'" readonly /></td>
...
/* Start amenities  */
/* echo "select * from bq_opfpmenudetail where fpno='".$fp_no."' AND subcatcode='oth' AND bill_status='1'"; */
$sbCI=mysql_query("select * from bq_opfpmenudetail where fpno='".$fp_no."' AND catcode='oth' AND bill_status='1' and itemcode!='55555'"); 
/* $sbCI=mysql_query("select * from bq_opfpmenudetail where fpno='".$fp_no."' AND bill_status='1'"); */
$nmI=mysql_num_rows($sbCI);
$totVlL=0;
while($rbC=mysql_fetch_array($sbCI)){
...
$output.='<tr id="">
	...
	<td style="text-align:center;" class="sourceonVAL"><input name="item_code[]" id="item_code" type="text"  class="textbox fstChUPPRCase expet" style="width:100px;margin:5px 0 0 0px"  value="'.$rbC['item_code'].'" readonly />
	<input name="sac[]" id="sac" type="hidden"  class="textbox fstChUPPRCase expet" style="width:100px;margin:5px 0 0 0px"  value="'.$rbC['sac'].'" readonly />
	</td>
...
echo $bkno.'&#'.$bkdate.'&#'.$rowb['guest_name'].'&#'.$roS['sess_name'].'&#'.$rov['venue_desc'].'&#'.$rob['bill_desc'].'&#'.$rowb['contact_person'].'&#'.$rowb['phone'].'&#'.$rowb['guaranted'].'&#'.$output.'&#'.$adv.'&#'.$totval.'&#'.$gst.'&#'.$gst.'&#'.$net.'&#'.$remar;
