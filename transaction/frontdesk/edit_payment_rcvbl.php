...
<?php 
	$sql=mysql_query("select * from ar_receipts where arreceipt_id='".$_GET['prvcId']."'");
...
?>		
	...
	<h3 style="text-align:center;font-size:14px;padding:10px;background:#0080C0;color:#fff;margin:1px 0 0 0;text-transform:uppercase;"><b>Payment Receivable</b></h3>
		<form id="taxTypes" name="taxTypes" enctype="multipart/form-data"  action="<?php echo $home_path;?>/action/update_ar_receipts.php" method="post" onsubmit="return formValid();" style="">
	<input name="arreceipt_id" id="arreceipt_id" type="hidden" value="<?php echo $row['arreceipt_id'];?>"/>
		<div>
			...
			<tbody>
				<tr>
						...
						<td valign="top"><input type="text" name="rcpt_date" id="rcpt_date" data-validation="required" class="input validate[required] datepicker1" value="<?php echo $row['rcpt_date'];?>"readonly />
						</td>
				</tr>
				<tr>
						<td width="" valign="top"><label>Receipt No<em>*</em></label></td>
						<td valign="top"><input type="text" name="rcpt_no" id="rcpt_no" data-validation="required" class="input validate[required]" value="<?php echo $row['rcpt_no'];?>" readonly />
						</td>
				</tr>		
				<tr>
				...
				<tr>
						...
						<td valign="top"><input type="text" name="amount" id="amount" data-validation="required" class="input validate[required,custom[number]] fstChUPPRCase" value="<?php echo $row['amount'];?>"/></td>
					...
					<tr>
						...
						<select name="pay_mode" id="pay_mode" data-validation="required" class="input validate[required] fstChUPPRCase" style="width:148px" onchange="payMDeCheque();">
							<option value="">--Select--</option>
							<option value="cash"<?php echo ($row['pay_mode']=='cash')?'selected':''; ?>>Cash</option>
							<option value="card"<?php echo ($row['pay_mode']=='card')?'selected':''; ?>>Card</option>
							<option value="cheque"<?php echo ($row['pay_mode']=='cheque')?'selected':''; ?>>Cheque</option>
							<option value="neft"<?php echo ($row['pay_mode']=='neft')?'selected':''; ?>>NEFT</option>
							</select>
						</td>
					</tr>
					<tr>
						<td width="" valign="top"><label>CC / Cheque #<em>*</em></label></td>
						<td valign="top"><input type="text" name="cheque_num" id="cheque_num" class="fstChUPPRCase" value="<?php echo $row['cheque_num'];?>" disabled /></td>
					</tr>
					<tr>
						<td width="" valign="top"><label>Cheque Date<em>*</em></label></td>
						<td valign="top"><input type="text" name="cheque_date" id="cheque_date" class="fstChUPPRCase datepicker" value="<?php echo $row['cheque_date'];?>" disabled /></td>
					</tr>
					<tr>
						<td width="" valign="top"><label>Remarks<em>*</em></label></td>
						<td valign="top"><input type="text" name="remarks" id="remarks" class="fstChUPPRCase" value="<?php echo $row['remarks'];?>" onblur="checkTaxCodeDesc();"/></td>
					</tr>
					</tbody>
				</table>
			</div>
	...
	<div style="margin:0 0 0 1px;">
		<button type="submit" id="add" class="buttExam_sngl bnkSbt frstChr" style="" onclick="popupBillAdv();"><img src="<?php echo $home_path;  ?>/images/saves.png" class="sbtBtnImg frstChr"/>&nbsp;&nbsp;<span class="btnUndLine">S</span>ubmit</button>
