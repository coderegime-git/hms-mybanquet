...
$sqR=mysql_query("select * from bq_opfpmenuhdr where fpno='".$_GET['fpno']."' AND bill_status='1'");
$roR=mysql_fetch_array($sqR); 

$sqRb=mysql_query("select * from bq_hallbooking where booking_no='".$roR['bkno']."' AND confirm_status='2'"); 
$rwB=mysql_fetch_array($sqRb);

?>	
<div id="addcustomer" class="col-sm-7 divBrd frmBgClr frmCentr" style="width:813px;height:502px;padding:0px;">
<form id="hotelDefi" name="hotelDefi" enctype="multipart/form-data"  action="<?php echo $home_path;?>/action/update_fpkot.php" method="post" class="" style="">	
	<h3 id="Userhd"><span style="color:#fff;"><?php if(isset($_POST['open_outlet'])){echo $_POST['open_outlet'];}?></span><span style=""><b>Edit K.O.T</b></span><span style="float:right;"><b><?php /* if(isset($sessCode)) { echo $sessCode;} */?></b></span></h3>
		...
		<table style="width:100%;border-right:1px solid #000;border-left:1px solid #000;border-top:1px solid #000;border-bottom:1px solid #000;margin:3px 0 0 0;" cellpadding="0" cellspacing="0" class="table" border="0" >
			...
			<td valign="top"><input name="fpno" id="fpno" type="text" class="input required textbox fstChUPPRCase tblImg" style="width:100px;margin:4px 0 0 0;" value="<?php echo $_GET['fpno'];?>" readonly />
						
			</td>
				
			<td style="width:54px;" valign="top"><label style="margin:3px 0 0 0;">Booking#</label></td>
			<td valign="top" style="width:50px;"><input name="booking_no" id="booking_no" type="text" class="input required textbox fstChUPPRCase tblImg" style="width:100px;margin:4px 0 0 0;"  value="<?php echo $rwB['booking_no'];?>" readonly />
			 </td>
			<td style="" valign="top"><label style="margin:3px 0 0 0;">Guest name</label></td>
			<td valign="top" style="width:50px;"><input name="guest_name" id="guest_name" type="text" class="input required textbox fstChUPPRCase tblImg" style="width:170px;margin:4px 0 0 0;"  value="<?php echo $rwB['guest_name'];?>" readonly />

			</td>
			<td style="" valign="top"><label style="margin:3px 0 0 0;">Pax</label></td>
			<td valign="top" style="width:50px;"><input name="guar_pax" id="guar_pax" type="text" class="input required textbox fstChUPPRCase tblImg" style="width:70px;margin:4px 0 0 0;" value="<?php echo $rwB['guaranted'];?>" readonly />
...
<thead class="tathead">
	<tr>
		...
		<th style="text-align:center;background-color:#0073B5;color:#fff;width:253px;">Item Desc</th>
		<th style="text-align:center;background-color:#0073B5;color:#fff;width:80px;">Qty</th>
		<th style="text-align:center;background-color:#0073B5;color:#fff;width:124px;">Item Rate</th>
		<th style="text-align:center;background-color:#0073B5;color:#fff;width:148px;">Value</th>
	</tr>
	</thead>
	<tbody id="tblRw" style="overflow:auto;height:360px;" class="tathead tatbody" >
<?php
$sqR=mysql_query("select * from bq_opkothdr where fpno='".$_GET['fpno']."' AND kotstatus='1'");
while($roR=mysql_fetch_array($sqR)){ 
?>
	<tr>
	...
	<input name="opkothdr_id[]" id="opkothdr_id" type="hidden" class="textbox codesUPPERCase" style="width:40px;margin:4px 0 0 0;" value="<?php echo $roR['opkothdr_id'];?>" />
	<input name="kotno" id="kotno" type="hidden" class="textbox codesUPPERCase" style="width:40px;margin:4px 0 0 0;" value="<?php echo $roR['kotno'];?>" />
	<input name="fp_no" id="fp_no" type="hidden" class="textbox codesUPPERCase" style="width:40px;margin:4px 0 0 0;" value="<?php echo $roR['fpno'];?>" />
		
		<td width="60"><input name="kot_itemcode[]" id="item_code1" type="text" class="textbox codesUPPERCase itemCde required" style="width:110px;margin:4px 0 0 0;" value="<?php echo $roR['item_code'];?>" />
		</td>
		<td width="60"><input name="kot_sac[]" id="kot_sac1" type="text" class="textbox codesUPPERCase required" style="width:90px;margin:4px 0 0 0;"  value="<?php echo $roR['sac'];?>" />
		<td width="" class="codesUPPERCase"><input name="kot_itemdesc[]" id="item_desc1" type="text" class="textbox codesUPPERCase required" style="width:250px;margin:4px 0 0 0;" value="<?php echo $roR['item_name'];?>" onblur="chkItemDesc(<?php echo '1' ?>);" /></td>
		<td width="40" class="fstChUPPRCase"><input name="kot_itemqty[]" id="item_qty1" type="text" class="textbox codesUPPERCase required" style="width:80px;margin:4px 0 0 0;" value="<?php echo $roR['item_qty'];?>" onkeypress="return pointNum(event)" onblur="chkItmQTy(<?php echo '1' ?>);" /></td>
		<td width="50" class="fstChUPPRCase"><input name="item_rate[]" id="item_rate1" type="text" class="textbox codesUPPERCase required" style="width:120px;margin:4px 0 0 0;" value="<?php echo $roR['item_rate'];?>" onkeypress="return pointNum(event)"  /></td>
		<td width="60" class="fstChUPPRCase">
		<input name="kot_itemval[]" id="item_val1" type="text" class="textbox codesUPPERCase lineTot required" style="width:120px;margin:4px 0 0 0;" value="<?php echo $roR['item_value'];?>" onkeypress="return pointNum(event)" readonly  />
		</td>
