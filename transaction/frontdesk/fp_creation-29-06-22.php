...
<table class="tree" style="font-size:10px;font-weight:bold;">
  ...
  <p style="color:#fff;background-color:#0073B5;text-align:Center;"> Menu items </p>
<?php 
$sql=mysql_query("select * from bq_itemmaster where status='1' group by itmnu_name");
while($row=mysql_fetch_array($sql)){ 
?>
    <tr class="treegrid-1 expanded"><td><?php echo strtoupper($row['itmnu_name']); ?></td></tr>
	<?php 
$sqll=mysql_query("select * from bq_itemmaster where itmnu_name='".$row['itmnu_name']."' and  status='1'");
$x=1;
while($roww=mysql_fetch_array($sqll)){
$x++;
?>
    <tr class="treegrid-2 treegrid-parent-1"><td><?php echo strtoupper($roww['item_name']); ?></td></tr>
<?php }} ?>
  </tbody>
</table>
</div>
...
<tr>
	...
	<th width="" style="text-align:center;background-color:#F5F5F5;font-size:12px;width:130px;">Preference</th>
