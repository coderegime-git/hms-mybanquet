...
if(isset($_GET['val']) && $_GET['val']!='') {
	...
	if(isset($_GET['dte']) && $_GET['dte']!=''){
$dte = $_GET['dte'];	
$dd=array_map('trim', explode('/',$dte));
// $cur=$dd[2].'/'.$dd[1].'/'.$dd[0];
$ddate=$dd[2].'-'.$dd[1].'-'.$dd[0];
	
$item_where= " where str_to_date(book_date,'%d/%m/%Y') = '$ddate' AND confirm_status!='7' AND confirm_status!='1' and log_status='1' and  venue like '%".$_GET['val']."%' and log_status='1' order by str_to_date(book_date,'%d/%m/%Y') ASC";	
$sql=mysql_query("select * from bq_hallbooking $item_where");	
	} else {
	
$item_where= " where  str_to_date(book_date,'%d/%m/%Y') >= '$cdate' AND confirm_status!='7' AND confirm_status!='1' and log_status='1' and guest_name like '%".$_GET['val']."%' OR phone like '%".$_GET['val']."%' OR venue like '%".$_GET['val']."%' OR booking_no like '%".$_GET['val']."%' and log_status='1' order by str_to_date(book_date,'%d/%m/%Y') ASC";
$sql=mysql_query("select * from bq_hallbooking $item_where");
}
}else{
...
<tr>
	...
	<td width="80" style="text-align:center;"><?php echo $row['audit_date']; ?></td>
...
<td width="80">
		<a onclick="clikval('<?php echo $row['booking_no']; ?>');" style="" class="Ckk">Log</a>&nbsp;
		</td>
