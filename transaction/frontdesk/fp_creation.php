...
/*$('input[name^=open_itemname]').keyup(function(){
	    rowid=($(this).attr("id")).substr(13);
		//alert(rowid);
		opnitm =parseFloat($(this).val()); 
		opnsac =parseFloat($(this).parent().next().next().next().next().find('input').val());
		opnit=$('#open_itemname'+rowid).val();
		opnsacc=$('#open_sac'+rowid).val();
		if(opnit!='' && opnsacc==''){
		   alert('Please fill the SAC code');
		   $('#open_sac'+rowid).focus();
		}
...
$('input[name^=pref]').on('keyup', function() {
			//pref =$(this).val();
			alert('hi');
...
function checkformSubmit() {
	...
	totpx=$('#tot_pax').val();	
	mnu=$('#menu').val();	
	opnitm=$('#open_itemname1').val();
	opnsub=$('#open_submenu1').val();
	opnrate=$('#open_itemrate1').val();
	opnsac=$('#open_sac1').val();
	opnqty=$('#opn_qty1').val();
		 if(bkn==''){
			...
			return false;
		}  else if(opnitm!="" && opnsub==""){
			alert('Please select the open item sub menu!.');
			$('#open_submenu1').focus();
			return false;
		} else if(opnitm!="" && opnqty==""){
			alert('Please enter the open item qty!.');
			$('#opn_qty1').focus();
			return false;
		} else if(opnitm!="" && opnrate==""){
			alert('Please enter the open item rate!.');
			$('#open_itemrate1').focus();
			return false;
		}  /* else{ */
			/* $('#hotelDefi').attr('action', '../../action/add_fp_creation.php');  
			$('#hotelDefi').submit(); */
	...
	$.ajax({
		...
		url:'  ../../action/selITMAMEnName.php',
			...
			success:function(data){
				/*  alert(data); */
				opt=data.split(',');				
				 $('#amen_itemcode'+c).val(opt[0]);
				 $('#amen_itemrate'+c).val(opt[1]);
				 $('#amen_itemqty'+c).focus();
			}
	});	
}
...
.overlay{
    ...
    background: rgba( 255, 255, 255, .8 ) 
                url("<?php echo $home_path;?>/images/loader1.gif") 
                50% 50% 
                no-repeat;
}
...
<div id="addcustomer" class="frmCentr divBrd frmBgClr" style="width:1200px;">
<!--<div id="addcustomer" class="frmCentr divBrd frmBgClr" style="width:1112px;overflow:auto;height:500px;">-->
	<h3 id="Userhd"><b>Banquet Menu </b></h3>
	<body class="bgBODY">
<form id="hotelDefi" name="hotelDefi" enctype="multipart/form-data" action="<?php echo $home_path;?>/action/add_fp_creation.php" method="post" class="" style="">
...
<tr>
	...
	<th style="text-align:left;background-color:#F5F5F5;width:80px;font-size:12px;">Id#</th>
...
/* $sqle=mysql_query("select * from bq_hallbooking where str_to_date(book_date,'%d/%m/%Y')>='$ctt' AND confirm_status='2' AND fp_status='' group by booking_no"); */
$sqle=mysql_query("select * from bq_hallbooking where str_to_date(book_date,'%d/%m/%Y')>='$ctt' AND confirm_status='2' order by str_to_date(book_date,'%d/%m/%Y') ASC");
$c=0;
...
<tr>
<td style="width:80px;"><input type="text" name="bookNo[]" id="bookNo<?php echo $c;?>" value="<?php echo $res['booking_no'];?>" style="width:80px;border:none;" class="textbox fstChUPPRCase actC" onclick="grpWse();" readonly /></td>
<td style="width:80px;"><input type="text" name="bookNoId[]" id="bookNoId<?php echo $c;?>" value="<?php echo $res['hallbook_id'];?>" style="width:80px;border:none;" class="textbox fstChUPPRCase actC"  readonly /></td>
<td style="width:150px;"><input type="text" name="grp_code[]" id="grp_code<?php echo $c;?>" value="<?php echo $res['guest_name'];?>" style="width:150px;border:none;" class="textbox fstChUPPRCase" readonly /></td>
<td style="width:150px;"><input type="text" name="grp_code[]" id="grp_code<?php echo $c;?>" value="<?php echo $res['book_date'];?>" style="width:150px;border:none;" class="textbox fstChUPPRCase" readonly /></td>
<td style="width:150px;border:none;"> <button type="button" onclick="selBQTFBCreat('<?php echo $res['booking_no'];?>','<?php echo $res['hallbook_id'];?>');" class="btnH" data-dismiss="modal">Click</button></td>
</tr>
<?php } ?>
</tbody>
...
<div id="myModal1" class="modal fade" role="dialog" style="padding:80px 0 0 0;width:500px;margin:0 auto;">
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      <div class="modal-header">
        <button type="button" class="" data-dismiss="modal">&times;</button>
        <h4 class="modal-title" style="text-align:center;font-weight:bold;">Department Instructions</h4>
...
<?php 
$sqldp=mysql_query("select * from bq_deptmt where status='1'");
$nmWs=mysql_num_rows($sqldp);
for($cc=1;$cc<=$nmWs;$cc++){
?>
...
<table style="float:left;width:50%;border-right:1px solid #ddd;margin:8px 0 0 0;" cellpadding="0" cellspacing="0" class="table tableS " border="0" >
<thead class="tathead">
<tr>
...
<tr>
	...
	<th width="" style="text-align:center;background-color:#F5F5F5;font-size:12px;width:240px;">Name</th>
...
<tr id="">
	<td style="text-align:center;" class="sourceonVAL">
	<input name="amen_itemcode[]" id="amen_itemcode<?php echo $cc;?>" type="hidden"  class="textbox fstChUPPRCase expet" style="width:150px;margin:5px 0 0 0px" value="" />
	...
	<td style="text-align:center;" class="sourceonVAL">
		<select name="amen_itemname[]" id="amen_itemname<?php echo $cc;?>" type="text"  class="textbox fstChUPPRCase expet" style="width:240px;margin:5px 0 0 0px;" value="" onchange="itmOthName(<?php echo $cc;?>);">
				<option value="">--Select--</option>
				<?php
				/* $sqle=mysql_query("select distinct item_code,item_name from bq_itemmaster where status='1' AND itmsub_cat='oth'"); */
				$sqle=mysql_query("select distinct item_code,item_name,itmsub_cat from bq_itemmaster where status='1' AND itmsub_cat IN('OTHERS','OT')");
				while($res=mysql_fetch_array($sqle)){
				?>
				<option value="<?php echo $res['item_code']  ?>" ><?php echo strtoupper ($res['item_name']); ?></option>
				<?php } ?>
		</select>
		</td>
	<td style="text-align:center;" class="sourceonVAL"><input name="amen_itemqty[]" id="amen_itemqty<?php echo $cc;?>" type="text"  class="textbox fstChUPPRCase expet" style="width:80px;margin:5px 0 0 0px" value="" /></td>
...
<div>
<?php
$sqlb=mysql_query("select * from bq_hallbooking where booking_no='".$_GET['bkNo']."' AND hallbook_id='".$_GET['bid']."'");
...
<td valign="top">
<input name="bk_date" id="bk_date" type="text" class="textbox fstChUPPRCase" style="width:210px"
value="<?php echo $rowb['book_date'];?>" readonly />
</td>
</tr>
...
<td valign="top"><input name="fp_no" id="fp_no" type="text" class="textbox fstChUPPRCase" style="width:210px" readonly /></td>
</tr>
<tr>
	...
	<td valign="top"><input name="guest_name" id="guest_name" type="text" data-validation="required" class="input validate[required] textbox fstChUPPRCase" style="width:210px" value="<?php echo $rowb['guest_name'];?>" readonly /></td>
		...
		<td valign="top"><input name="venue" id="venue" type="text" data-validation="required" class="input validate[required] textbox fstChUPPRCase" value="<?php echo $rov['venue_desc'];?>" style="width:210px" readonly /></td>
		</tr>
		...
		<tr>
			...
			<td valign="top">
			<input  name="exppax" id="exppax" type="text" data-validation="required" class="input validate[required] textbox fstChUPPRCase" value="<?php echo $rowb['expected'];?>" style="width:70px;background-color:#e9ecef;" readonly />
			<span class="spanClr">Gaurn Pax</span>&nbsp;
			<input name="gaupax" id="gaupax" type="text" data-validation="required" class="input validate[required] textbox fstChUPPRCase" value="<?php echo $rowb['guaranted'];?>" style="width:70px;background-color:#e9ecef;" readonly />
			</td>
		</tr>
	...
	<option value="">--Select--</option>
	<?php
	$sqle=mysql_query("select distinct itmnu_code,itmnu_name from bq_itemmaster where status='1' AND itmnu_code!=''");
	while($res=mysql_fetch_array($sqle)){
	?>
	<option value="<?php echo $res['itmnu_code']  ?>" ><?php echo strtoupper($res['itmnu_name']); ?></option>
	<?php } ?>
	</select>
	</td>-->
	<td valign="top">
	<select name="menu" id="menu" style="font-size:12px;width:210px" onChange="selMenuCode();" class="wagRw1 textbox">
			<option value="">--Select--</option>
			<?php
		  $sqle=mysql_query("select distinct menu_code,menu_name from bq_menumaster where status='1' group by menu_code");
			while($res=mysql_fetch_array($sqle)){ 
			?>
			<option value="<?php  echo $res['menu_code']  ?>" ><?php  echo strtoupper($res['menu_name']);  ?></option>
			<?php  }  ?>
	</select>
	</td>
	</tr>
		...
		<tr>
			...
			<td valign="top">
			<input type="time" name="arrtime" id="arrtime" class="fstChUPPRCase textbox" style="width:95px" value="" onkeyup="arrtTime();" >
			<span class="spanClr">Pick</span>&nbsp;
			<input name="pictime" id="pictime" type="time" class="textbox fstChUPPRCase" style="width:95px;margin:0 0 0 -8px;" value='' onkeyup="pictTime();" />
			</td>
		</tr>
		<tr>
			<td width="" valign="top"><label>Serv</label></td>
			<td valign="top">
			<input name="sertime" id="sertime" type="time" class="textbox fstChUPPRCase" style="width:95px;margin:0 0 0 -8px;" value='' onkeyup="sertTime();" />
			</td>
		</tr>
		<!--<tr>
		...
		<tr>
			...
			<td valign="top"><textarea cols="34" rows="2" name="remarks" id="remarks"  style="text-transform:uppercase;font-size:12px;"><?php echo $rowb['remarks'];?></textarea></td>
		</tr>
		<tr>
				<td width="" valign="top"><label>Sign Board</label></td>
				<td valign="top"><textarea cols="34" rows="" name="sign_board" id="sign_board" value="" style="text-transform:uppercase;font-size:12px;margin:0px 0 0 0;"></textarea></td>
		</tr>
<?php 
...
?>
		<tr>
			<td width="" valign="top"><label>Hall Charge</label></td>
			<td valign="top">
			...
			<span class="spanClr">Hall Tax&nbsp;&nbsp;</span>&nbsp;
			<input name="halltax_chg" id="halltax_chg" type="text" onkeypress="return pointNum(event)" class="textbox fstChUPPRCase" style="width:60px;margin:0 0 0 -8px;" value='<?php echo $rowb['hall_rate']; ?>' />
		...
		<tr>
			<td width="" valign="top"><label>Food Charge</label></td>
			<td valign="top">
			...
			<span class="spanClr">Food Tax</span>&nbsp;
			<input name="ratetax_chg" id="ratetax_chg" type="text" onkeypress="return pointNum(event)" class="textbox fstChUPPRCase" style="width:60px;margin:0 0 0 0px;" value='<?php echo $rowb['plan_rate']; ?>'/>
...
<!--<tr>
<td width="" valign="top"><label>Department</label></td>
<td valign="top"><input name="department" id="department" type="text" class="textbox fstChUPPRCase" style="width:210px"/></td>
</tr>
<tr>
<td width="" valign="top"><label>Remarks</label></td>
<td valign="top"><textarea cols="35" rows="1" name="dept_remarks" id="dept_remarks" value="" style="text-transform:uppercase;font-size:12px;"></textarea></td>
</tr>-->
...
<div class="" style="margin-top:0px;width:274px;float:right;height:300px;">
...
<!--<table class="tree" style="font-size:10px;font-weight:bold;">
  <tbody>
  <p style="color:#fff;background-color:#0073B5;text-align:Center;"> Menu items </p>
<?php 
...
$x++;*/
?>
    <tr class="treegrid-1 expanded"><td><?php echo strtoupper($row['menu_name']); ?></td></tr>
...
<table class="tree" style="font-size:10px;font-weight:bold;">
  <tbody>
<?php 
$sql=mysql_query("SELECT itmsubmnu_code FROM `bq_itemmaster` where status='1' group by itmsubmnu_code");
$x=0;
while($row=mysql_fetch_array($sql)){ 
...
$x++;
?>
    <tr class="treegrid-<?php echo  $x;  ?>"><td><?php echo strtoupper($rowL['submenu_name']); ?></td></tr>
	<?php 
...
?>
    <tr class="treegrid-a<?php echo  $x;  ?> treegrid-parent-<?php echo  $x;  ?>"><td><?php echo strtoupper($roww['item_name']); ?></td></tr>
<?php }} ?>
  </tbody>
</table>
</div>
...
<tr>
	<th width="" style="text-align:center;background-color:#F5F5F5;font-size:12px;width:337px;">Item name</th>
	...
	<th width="" style="text-align:center;background-color:#F5F5F5;font-size:12px;width:190px;">Sub Menu</th>
	<!--<th width="" style="text-align:center;background-color:#F5F5F5;font-size:12px;width:140px;">SAC code</th>
...
?>
<tr id="">
	<td style="text-align:center;" class="sourceonVAL"><input name="open_itemname[]" id="open_itemname<?php echo $cc; ?>" type="text"  class="textbox fstChUPPRCase expet" style="width:337px;margin:5px 0 0 0px" value="" /></td>
	<!--<td style="text-align:center;" class="sourceonVAL"><input name="opn_qty[]" id="opn_qty<?php echo $cc; ?>" type="text"  class="textbox fstChUPPRCase expet" style="width:50px;margin:5px 0 0 0px" value="" /></td>
	<td style="text-align:center;" class="sourceonVAL"><input name="open_itemrate[]" id="open_itemrate<?php echo $cc; ?>" type="text"  class="textbox fstChUPPRCase expet" style="width:80px;margin:5px 0 0 0px" value="" /></td>-->
	<td style="text-align:center;" class="sourceonVAL">
	<select name="open_submenu[]" id="open_submenu<?php echo $cc; ?>" type="text"  class="textbox fstChUPPRCase expet" style="width:190px;margin:5px 0 0 0px" value="">
				<option value="">--Select--</option>
				<?php
				$sqle=mysql_query("select distinct grpcode,grpname from bq_grpcode where status='1'");
				while($res=mysql_fetch_array($sqle)){
	...
	<!--<td style="text-align:center;" class="sourceonVAL"><input name="open_sac[]" id="open_sac<?php echo $cc; ?>" type="text"  class="textbox fstChUPPRCase expet" style="width:140px;margin:5px 0 0 0px" value="" /></td>-->
</tr>	
<?php 
}
?>	
...
<table style="float:left;margin:133px 0 0 0;">
<tr>
<td>

<button type="button" id="submit" class="btnHV bnkSbt frstChr submit" style="" data-toggle="modal" data-target="#myModal1" ><span class="btnUndLine" style="">D</span>epartment instructions</button>
</td>
<td>
...
<div class="col-md-12  responsive nowrap " style=" padding-left:3px;">
		...
		<!--<a href="#" target="_blank"><button type="button" id="hallsts" class="btn btn-primary btn-sm btn-responsive" style="" onclick="hall_sts()"><img src="../../images/clear-icon.png" class="sbtBtnImg" style="width:20px;height:20px;" />&nbsp;&nbsp;<span class="btnUndLine">U</span>pdate</button></a>-->
