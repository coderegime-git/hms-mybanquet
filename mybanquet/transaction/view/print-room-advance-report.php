...
$sqlRD=mysql_query("select * from  room_advance where roomadv_id='".$_GET['advRDId']."'");
$rowRd=mysql_fetch_array($sqlRD); 
$remarks=$rowRd['remarks'];

$curDte=$rowRd['cur_date'];
$rcptNo=$rowRd['receipt_no'];
$roomNo=$rowRd['room_no'];
$gustNme=$rowRd['guest_name'];
$amt=sprintf("%01.2f",$rowRd['amount']);
$payMde=$rowRd['pay_mode'];




$sqlG=mysql_query("select * from guest_register gr, guest_trans gt where gr.room_no='".$rowRd['room_no']."' AND gt.room_no='".$rowRd['room_no']."' AND gr.guestreg_id=gt.reg_num AND gt.bill_status='1'");
/* $sqlG=mysql_query("select * from guest_register where room_no='".$rowRd['room_no']."'"); */
$rowG=mysql_fetch_array($sqlG);
$grcNo=$rowG['grc_number'];
$title=ucfirst($rowG['title']);

$NETpAYy=sprintf("%01.2f",$rowRd['amount']);
$aWords = new Currency();
$finTot =$NETpAYy;
