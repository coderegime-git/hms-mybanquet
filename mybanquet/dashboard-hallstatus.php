...
<?php 
$toDate=$adtCurDt;
$sqlAR=mysql_query("select count(distinct booking_no) AS todayDep from bq_hallbooking where confirm_status='2'"); 
...
<tr><td style="width:35px;margin:0 0 0 5px;background-color:#C3C3C3;color:#000;border:1px solid #000;">Booking#</td><td style="width:30px;background-color:#C3C3C3;color:#000;">Gst Name</td><td style="width:50px;background-color:#C3C3C3;color:#000;border:1px solid #000;">Func Date</td><td style="width:50px;background-color:#C3C3C3;color:#000;border:1px solid #000;">venue</td>
</tr>
<?php 
	$sqlR=mysql_query("select * from bq_hallbooking where confirm_status='2'"); 
	$pax=0;$adTpax=0;$chDpax=0;
	while($rowR=mysql_fetch_array($sqlR)){
		
		$gstST= mb_strimwidth($rowR['guest_name'], 0, 10, '...');
		
?>
<tr><td style="width:20px;margin:0 0 0 5px;border-right:1px solid #000;"><a href="<?php echo $home_path;?>/transaction/frontdesk/edit-hall-status.php?roomBk=<?php echo $rowR['booking_no']; ?>&rmBkID=<?php echo $rowR['hallbook_id']; ?>" target="_blank" title="Click here"><?php echo $rowR['booking_no']; ?></a></td><td style="width:60px;border-right:1px solid #000;"><?php echo $gstST; ?></td><td style="width:50px;border-right:1px solid #000;"><?php echo $rowR['book_date']; ?></td><td style="width:50px;border-right:1px solid #000;"><?php echo $rowR['venue']; ?></td>
</tr>
	<?php } ?>
</table></li>
			</ul>
...
$sqlAR=mysql_query("select count(distinct booking_no) AS fortheDy from bq_hallbooking where str_to_date(book_date,'%d/%m/%Y') = '$add' AND confirm_status='2'"); 
$rowAr=mysql_fetch_array($sqlAR);
...
<ul>
	...
	<ul>
	<li style="color:#af0a05;background-color:#000;"><table class="table dsWrpTbl" style="border:1px solid #000;">
<tr><td style="width:35px;margin:0 0 0 5px;background-color:#C3C3C3;color:#000;border:1px solid #000;">Booking#</td><td style="width:30px;background-color:#C3C3C3;color:#000;">Gst Name</td><td style="width:50px;background-color:#C3C3C3;color:#000;border:1px solid #000;">Func Date</td><td style="width:50px;background-color:#C3C3C3;color:#000;border:1px solid #000;">venue</td>
</tr>
<?php
$sqlAR=mysql_query("select * from bq_hallbooking where str_to_date(book_date,'%d/%m/%Y')= '$add' AND confirm_status='2'"); 
...
<?php 
$sqlAR=mysql_query("select * from bq_hallbooking where str_to_date(book_date,'%d/%m/%Y') >= '$frm' AND str_to_date(book_date,'%d/%m/%Y') <= '$tod' AND confirm_status='2'"); 
while($rowR=mysql_fetch_array($sqlAR)){
$gstST= mb_strimwidth($rowR['guest_name'], 0, 10, '...');
?>
<tr><td style="width:20px;margin:0 0 0 5px;border-right:1px solid #000;"><a href="<?php echo $home_path;?>/transaction/frontdesk/edit-hall-status.php?roomBk=<?php echo $rowR['booking_no']; ?>&rmBkID=<?php echo $rowR['hallbook_id']; ?>" target="_blank" title="Click here"><?php echo $rowR['booking_no']; ?></a></td><td style="width:60px;border-right:1px solid #000;"><?php echo $gstST; ?></td><td style="width:50px;border-right:1px solid #000;"><?php echo $rowR['book_date']; ?></td><td style="width:50px;border-right:1px solid #000;"><?php echo $rowR['venue']; ?></td>
...
function showGridView() {
	...
	if(fromdate!="" && todate!="")
	{
		document.location="dashboard-hallstatus.php?fromdate="+fromdate+"&todate="+todate+"&ven="+ven;
	}
}
...
function dashNext(){
	...
	$.ajax({
		type:'GET',
		url:' action/seldashBrdBQThall.php',
			data:{
			frm:frm 
			},
...
function dashprevious(){
	...
	$.ajax({
		type:'GET',
		url:' action/seldashBrdNxtBQTHall.php',
			data:{
			frm:frm 
			},
			success:function(data){
...
<div style="margin:0px 0 0 0px;background-color:#7b0e0e;color:#fff;" >
...
/* echo $bgcolor; */
?>
<td style="text-align:center;width:20px;background-color:<?php echo $bgcolor; ?>;color:#fff;" ><a href="<?php echo $home_path;?>/transaction/frontdesk/edit-hall-status.php?roomBk=<?php echo $roD['booking_no']; ?>&rmBkID=<?php echo $roD['hallbook_id']; ?>" data-toggle="tooltip" title="<?php echo 'BK#:'.$roD['booking_no'].','.strtoupper('  GUEST:'.$roD['guest_name']).',  PAX: '.$sqb['guaranted'];?>">&nbsp;</a></td>
<?php }else{ ?>
...
<div id="dshBrdShw" style="width:100%;height:463px;overflow:auto;">
