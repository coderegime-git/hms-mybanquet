...
{
document.location="function-report.php?fromdate="+fromdate+"&todate="+todate+"&funct="+funct;
}
...
<td><label style="width:70px;"><b>Function</b>&nbsp;&nbsp;</label></td>
...
<td style="text-align:center;" class="sourceonVAL">
	<?php $sqlBS=mysql_query("select distinct func_code,func_desc from bq_function where status='1'"); ?>
		<select name="funct" id="funct" class="fstChUPPRCase" style="width:120px;float:left;font-size:12px;" onChange="selFunctionName('<?php echo $cc;?>');">
		<option value="">--Select--</option>
		<option value="">All</option>
		<?php  while($rowBS=mysql_fetch_array($sqlBS)) {  ?>
		<?php if($rowBS['func_code']==$_GET['funct']){	?>
...
<td>
	<a href="<?php echo $home_path ?>/reports/checkout/xt_funcsales_report_xls.php?fromdate=<?php echo $_GET['fromdate']?>&todate=<?php echo $_GET['todate']?>&funct=<?php echo $_GET['funct']?>" style="margin:0px 0 0 62px;color:#000;font-size:13px;font-weight:bold;"><button type="button" id="pdf" style="margin:0 0 0 44px;" class="myButeXL btnn"><img src="../../images/excel1.png"  class="sbtBtnImg"/>&nbsp;Export&nbsp;</button></a>
</td>
<td>
...
<table class="table table-condensed table-hover table-striped table-bordered" cellpadding="0" cellspacing="0" border="1" class="table" style="margin:0px 0 0px -5px;text-align:center;font-size:12px;">
	...
	<td colspan="35" style="text-align:center;"><h3 class="viewDTT"><b>FUNCTION SALES REPORT</b></h3><b></b></td>
	</tr>
</table>
...
if(isset($_GET['fromdate'],$_GET['todate'])) {
	...
	$tod=$to[2].'-'.$to[1].'-'.$to[0];
if(isset($_GET['funct']) && $_GET['funct']!=''){
$sdep=mysql_query("select distinct func_code,func_desc from bq_function where func_code='".$_GET['funct']."' AND status='1'");
}else{
$sdep=mysql_query("select distinct func_code,func_desc from bq_function where status='1'");	
}
while($rwp=mysql_fetch_array($sdep)){
$item_where=" where str_to_date(bill_date,'%d/%m/%Y') >= '$frm' AND str_to_date(bill_date,'%d/%m/%Y') <= '$tod' AND funct='".$rwp['func_code']."' order by opbillhdr_id ASC";
...
if(mysql_num_rows($sql)>0) {
	?>
<tr>
<td style="text-align:left;width:120px;color:#FF0034;font-weight:bold;" colspan="24"><?php echo strtoupper($rwp['func_desc']); ?></td>
</tr>
<?php
...
<?php
if(isset($_GET['funct']) && $_GET['funct']!=''){
$sqlbl=mysql_query("select SUM(nontaxableamt)AS nontax,SUM(taxableamt)AS taxable,SUM(discamt)AS discable,SUM(billamt)AS billable,SUM(gpax)AS tGpax from bq_opbillhdr where str_to_date(bill_date,'%d/%m/%Y') >= '$frm' AND str_to_date(bill_date,'%d/%m/%Y') <= '$tod' AND funct='".$_GET['funct']."'  AND bill_status!='3'");
}else{
...
while($rowtS=mysql_fetch_array($sqlTS)){
if(isset($_GET['funct']) && $_GET['funct']!=''){
$sqL=mysql_query("select SUM(item_total)AS itmTot  from bq_opbillhdtl where grpcode='".$rowtS['grpcode']."' AND funct='".$_GET['funct']."' AND str_to_date(bill_date,'%d/%m/%Y') >= '$frm' AND str_to_date(bill_date,'%d/%m/%Y') <= '$tod' AND bill_status!='3'");
}else{
...
if(isset($_GET['funct']) && $_GET['funct']!=''){
$scg=mysql_query("select sum(cgst)AS cgst from bq_opbillhdr where cgst>0 AND str_to_date(bill_date,'%d/%m/%Y') >= '$frm' AND str_to_date(bill_date,'%d/%m/%Y') <= '$tod' AND funct='".$_GET['funct']."'  AND bill_status!='3'");
}else{
...
if(isset($_GET['funct']) && $_GET['funct']!=''){
$ssg=mysql_query("select sum(sgst)AS sgst from bq_opbillhdr where sgst>0 AND str_to_date(bill_date,'%d/%m/%Y') >= '$frm' AND str_to_date(bill_date,'%d/%m/%Y') <= '$tod' AND funct='".$_GET['funct']."' AND bill_status!='3'");
}else{
