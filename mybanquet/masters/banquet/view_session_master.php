...
jQuery(document).ready(function(){
    ...
    if(typeof shortcut !== 'undefined') {
        shortcut.add("Ctrl+A", function() { 
            window.location.href = "session_master_bqt.php";
        }); 
...
<div class="mypay-container">
    ...
    <div class="mypay-actions-bar">
        <a href="session_master_bqt.php" class="btn-mypay-add" id="add" title="Add Session (Ctrl+A)">
    ...
    <table class="mypay-table" cellpadding="0" cellspacing="0">
        <thead>
            <tr class="banner-row">
                <th colspan="6">VIEW SESSION DETAILS</th>
            </tr>
            <tr class="header-row">
                <th style="width: 15%;">Code</th>
                <th style="width: 30%;">Name</th>
                <th style="width: 18%;">From Time</th>
                <th style="width: 18%;">To Time</th>
        ...
        <tbody>
        <?php 
        $sql = mysql_query("select * from bqt_session order by sess_id asc");
        if($sql && is_resource($sql) && mysql_num_rows($sql) > 0) {
            while($row = mysql_fetch_array($sql)) {
                $status = ($row['status'] == 1) ? "Active" : "Passive";
        ?>
            <tr>
                <td><?php echo htmlspecialchars($row['sess_code']); ?></td>
                <td><?php echo htmlspecialchars($row['sess_name']); ?></td>
                <td><?php echo htmlspecialchars($row['from_time']); ?></td>
                <td><?php echo htmlspecialchars($row['to_time']); ?></td>
                <td><?php echo $status; ?></td>
                <td>
                    <a href="edit_session.php?sesid=<?php echo $row['sess_id']; ?>" class="mypay-edit-btn" title="Edit">
