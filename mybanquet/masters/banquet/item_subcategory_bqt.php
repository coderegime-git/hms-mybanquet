...
$(document).ready(function(){
    if(typeof jQuery.fn.validationEngine !== 'undefined') {
        $("#itemSubCatForm").validationEngine();
    }
    $("#msgFo").fadeOut(5000);
    $("#subcat_code").focus();
    ...
    if(typeof shortcut !== 'undefined') {
        shortcut.add("Ctrl+S", function() { 
            $('#itemSubCatForm').attr('action', '../../action/add_itemSubcate_bqt.php');  
            $('#itemSubCatForm').submit(); 
        }); 
...
function checkSubCategoryCode() {
    var subcat_code = $('#subcat_code').val().trim();
    if(subcat_code == '') return;
    $.ajax({
        type: 'GET',
        url: '../../action/repeatBqtItemSubCatCode.php',
        data: { subcat_code: subcat_code },
        success: function(data){
            if(data == 1){
                alert('Sub Category code already exists!');
                $('#subcat_code').val('').focus();
            }
        }
    });
}
...
<div class="mypay-container">
    ...
    <div class="mypay-card">
        ...
        <form id="itemSubCatForm" name="itemSubCatForm" enctype="multipart/form-data" action="<?php echo $home_path;?>/action/add_itemSubcate_bqt.php" method="post">
            <div class="mypay-card-body">
                <table class="mypay-form-table" cellpadding="0" cellspacing="0">
                    <tbody>
                        <tr>
                            <td class="mypay-label-cell">
                                <label for="subcat_code">Sub Category Code<span class="req-star">*</span> :</label>
                            </td>
                            <td class="mypay-input-cell">
                                <input type="text" name="subcat_code" id="subcat_code" data-validation-engine="validate[required]" class="mypay-input validate[required]" style="text-transform:uppercase;" onblur="checkSubCategoryCode();" required />
                            </td>
                        </tr>
                        <tr>
                            <td class="mypay-label-cell">
                            ...
                            <td class="mypay-input-cell">
                                <input type="text" name="subcat_name" id="subcat_name" data-validation-engine="validate[required]" class="mypay-input validate[required]" onblur="checkSubCategoryName();" required />
                            </td>
                        </tr>
                        <tr>
                            <td class="mypay-label-cell">
                            ...
                            <td class="mypay-input-cell">
                                <select name="cat_code" id="cat_code" data-validation-engine="validate[required]" class="mypay-input validate[required]" required>
                                    ...
                                    ?>
                                        <option value="<?php echo htmlspecialchars($resulte['cat_code']); ?>">
