...
$(document).ready(function(){
 ...
 shortcut.add("Ctrl+S",function() { 
	 $('#taxTypes').attr('action', '../../action/update_market_segment.php');  
	 $('#taxTypes').submit(); 
}); 
...
<?php 
	$sql=mysql_query("select * from bq_marketseg where marseg_id='".$_GET['ms_id']."'");
	$row=mysql_fetch_array($sql);
?>				
	<div id="addcustomer" class="frmCentr divBrd frmBgClr" style="width:468px;">
	<h3 id="Userhd"><b>Market Segment(BQT)</b></h3>
		<form id="taxTypes" name="taxTypes" enctype="multipart/form-data"  action="<?php echo $home_path;?>/action/update_market_segment.php" method="post" class="" style="">
		<input type="hidden" name="msid" id="msid" value="<?php echo $_GET['ms_id'];?>"/>
		<div>
			...
			<tbody>
					<tr>
						...
						<td valign="top"><input type="text" name="segment_code" id="segment_code" data-validation="required" class="textbox input validate[required] codesUPPERCase tobCode" value="<?php echo $row['mscode'];?>" readonly /></td>
					</tr>
					<tr>
						<td width="" valign="top"><label>Segment Name<em>*</em></label></td>
						<td valign="top"><input type="text" name="segment_name" id="segment_name" data-validation="required" class="textbox input validate[required] fstChUPPRCase" value="<?php echo $row['msname'];?>" onblur="checkMarketsegmentname();"/></td>
