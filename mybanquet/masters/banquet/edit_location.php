...
<?php 
	$sql=mysql_query("select * from bq_location where locat_id='".$_GET['locMd']."'");
	$row=mysql_fetch_array($sql);
?>			
	<div id="addcustomer" class="frmCentr divBrd frmBgClr" style="width:468px;">
	<h3 id="Userhd"><b>Location Master(BQT)</b></h3>
		<form id="taxTypes" name="taxTypes" enctype="multipart/form-data"  action="<?php echo $home_path;?>/action/update_location.php" method="post" class="" style="">
		<input type="hidden" name="locat_id" id="locat_id" value="<?php echo $_GET['locMd'];?>"/>
		<div>
			<table cellpadding="0" cellspacing="0" class="table" border="0" style="margin:4px 0 0 0;" >
			<tbody>
					<tr>
						<td width="" valign="top"><label>Location Code <em>*</em></label></td>
						<td valign="top"><input type="text" name="loc_code" id="loc_code" data-validation="required" class="textbox input validate[required] codesUPPERCase tobCode" value="<?php echo $row['loc_code'];?>" onblur="checkBusinessSource();" readonly /></td>
					</tr>
					<tr>
						<td width="" valign="top"><label>Location Name<em>*</em></label></td>
						<td valign="top"><input type="text" name="loc_desc" id="loc_desc" data-validation="required" class="textbox input validate[required] fstChUPPRCase" value="<?php echo $row['loc_desc'];?>" onblur="checkBusinessDesc();"/></td>
	...
	<div style="margin:0 0 0 0px;">
		...
		<a href="view_location.php"><button type="button" id="update" class="buttExam_sngl bnkSbt" onclick="return checkPropertyMasterq();"><img src="<?php echo $home_path; ?>/images/audit.png" class="sbtBtnImg "/>&nbsp;&nbsp;<span class="btnUndLine">V</span>iew</button></a>
