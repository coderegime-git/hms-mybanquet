...
$(document).ready(function(){
 ...
 shortcut.add("Ctrl+S",function() { 
	 $('#taxTypes').attr('action', '../../action/update_dapartment_bqt.php');  
	 $('#taxTypes').submit(); 
}); 
...
<?php 
	$sql=mysql_query("select * from bq_deptmt where deptmt_id='".$_GET['deptmt_id']."'");
	$row=mysql_fetch_array($sql);
?>				
	<div id="addcustomer" class="frmCentr divBrd frmBgClr" style="width:468px;">
	<h3 id="Userhd"><b>Departments(BQT)</b></h3>
		<form id="taxTypes" name="taxTypes" enctype="multipart/form-data"  action="<?php echo $home_path;?>/action/update_department_bqt.php" method="post" class="" style="">
		<input type="hidden" name="deptmt_id" id="deptmt_id" value="<?php echo $_GET['deptmt_id'];?>"/>
		<div>
			<table cellpadding="0" cellspacing="0" class="table" border="0" style="margin:4px 0 0 0;" >
			<tbody>
					<tr>
						<td width="" valign="top"><label>Department Code <em>*</em></label></td>
						<td valign="top"><input type="text" name="dept_code" id="dept_code" data-validation="required" class="textbox input validate[required] codesUPPERCase tobCode" value="<?php echo $row['dept_code'];?>" readonly /></td>
					</tr>
					<tr>
						<td width="" valign="top"><label>Department Name<em>*</em></label></td>
						<td valign="top"><input type="text" name="dept_name" id="dept_name" data-validation="required" class="textbox input validate[required] fstChUPPRCase" value="<?php echo $row['dept_name'];?>" onblur="checkDepartmentName();"/></td>
...
<table style="border-left:1px solid #ddd;" class="table">
	...
	<div style="margin:0 0 0 0px;">
		...
		<a href="view_department_bqt.php"><button type="button" id="update" class="buttExam_sngl bnkSbt" onclick="return checkPropertyMasterq();"><img src="<?php echo $home_path; ?>/images/audit.png" class="sbtBtnImg "/>&nbsp;&nbsp;<span class="btnUndLine">V</span>iew</button></a>
