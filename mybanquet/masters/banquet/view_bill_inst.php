...
jQuery(document).ready(function(){
    ...
    if(typeof shortcut !== 'undefined') {
        shortcut.add("Ctrl+A", function() { 
            window.location.href = "billing_instruction_bqt.php";
        }); 
...
<div class="mypay-container">
    ...
    <div class="mypay-actions-bar">
        <a href="billing_instruction_bqt.php" class="btn-mypay-add" id="add" title="Add Billing Instruction (Ctrl+A)">
            <span class="mypay-icon-plus"><i class="fa fa-plus"></i></span>
            <span>Add Billing Instruction</span>
        </a>
    ...
    <table class="mypay-table" cellpadding="0" cellspacing="0">
        <thead>
            <tr class="banner-row">
                <th colspan="4">VIEW BILLING INSTRUCTION</th>
            </tr>
            <tr class="header-row">
        ...
        <tbody>
        <?php 
        $sql = mysql_query("select * from bq_billinstruc order by billinst_id asc");
        if($sql && is_resource($sql) && mysql_num_rows($sql) > 0) {
            while($row = mysql_fetch_array($sql)) {
                $status = ($row['status'] == 1) ? "Active" : "Passive";
        ?>
            <tr>
                <td><?php echo htmlspecialchars($row['bill_code']); ?></td>
                <td><?php echo htmlspecialchars($row['bill_desc']); ?></td>
                <td><?php echo $status; ?></td>
                <td>
                    <a href="edit_billinst.php?blMd=<?php echo $row['billinst_id']; ?>" class="mypay-edit-btn" title="Edit">
        ...
        ?>
            <tr>
                <td colspan="4" style="padding: 16px; color: #777; text-align: center; font-size: 13px;">
                    No Billing Instruction records found
                </td>
            </tr>
        <?php } ?>
        </tbody>
