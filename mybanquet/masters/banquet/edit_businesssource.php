...
$bssrc_id = isset($_GET['bssrc_id']) ? mysql_real_escape_string($_GET['bssrc_id']) : '';
$sql = mysql_query("select * from bq_bssource where bssrc_id='$bssrc_id'");
...
$(document).ready(function(){
    if(typeof jQuery.fn.validationEngine !== 'undefined') {
        $("#businessSourceForm").validationEngine();
    }
    $("#msgFo").fadeOut(5000);
    $("#bs_name").focus();
    ...
    if(typeof shortcut !== 'undefined') {
        shortcut.add("Ctrl+S", function() { 
            $('#businessSourceForm').attr('action', '../../action/update_business_source.php');  
...
function checkBusinessSourceName(){
    var source_name = $('#bs_name').val().trim();
    if(source_name == '') return;
    $.ajax({
        type: 'GET',
        url: '../../action/repeaBusinessSourceDesc.php',
        data: { source_desc: source_name },
        success: function(data){
            if(data == 1){
                alert('Business Source Name already exists!');
            }
        }
    });
}
...
<div class="mypay-container">
    ...
    <div class="mypay-card">
        ...
        <form id="businessSourceForm" name="businessSourceForm" enctype="multipart/form-data" action="<?php echo $home_path;?>/action/update_business_source.php" method="post">
            <input type="hidden" name="bssrc_id" id="bssrc_id" value="<?php echo htmlspecialchars($bssrc_id); ?>" />
                    ...
                    <tbody>
                        <tr>
                            <td class="mypay-label-cell">
                                <label for="bs_code">Business Source Code<span class="req-star">*</span> :</label>
                            </td>
                            <td class="mypay-input-cell">
                                <input type="text" name="bs_code" id="bs_code" class="mypay-input" value="<?php echo htmlspecialchars(isset($row['bs_code']) ? $row['bs_code'] : ''); ?>" readonly />
                            </td>
                        </tr>
                        <tr>
                            <td class="mypay-label-cell">
                                <label for="bs_name">Business Source Name<span class="req-star">*</span> :</label>
                            </td>
                            <td class="mypay-input-cell">
                                <input type="text" name="bs_name" id="bs_name" data-validation-engine="validate[required]" class="mypay-input validate[required]" value="<?php echo htmlspecialchars(isset($row['bs_name']) ? $row['bs_name'] : ''); ?>" onblur="checkBusinessSourceName();" required />
                            </td>
                        </tr>
                        <tr>
                            <td class="mypay-label-cell">
            ...
            <div class="mypay-card-footer">
                ...
                <a href="view_business_source.php" class="btn-mypay-action" title="View (Ctrl+V)">
