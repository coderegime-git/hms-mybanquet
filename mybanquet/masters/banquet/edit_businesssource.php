...
function checkBusinessSourceName(){
	...
	$.ajax({
		type:'GET',
		url:'  ../../action/repeatBusinessSourceName.php',
			data:{
...
<?php 
	$sql=mysql_query("select * from bq_bssource where bssrc_id='".$_GET['bssrc_id']."'");
	$row=mysql_fetch_array($sql);
?>				
	<div id="addcustomer" class="frmCentr divBrd frmBgClr" style="width:468px;">
	<h3 id="Userhd"><b>Business Source(BQT)</b></h3>
		...
		<input type="hidden" name="bssrc_id" id="bssrc_id" value="<?php echo $_GET['bssrc_id'];?>"/>
		<div>
			<table cellpadding="0" cellspacing="0" class="table" border="0" style="margin:4px 0 0 0;" >
			<tbody>
					<tr>
						<td width="" valign="top"><label>Business Source Code<em>*</em></label></td>
						<td valign="top"><input type="text" name="bs_code" id="bs_code" data-validation="required" class="textbox input validate[required] codesUPPERCase tobCode" value="<?php echo $row['bs_code'];?>" readonly /></td>
					</tr>
					<tr>
						<td width="" valign="top"><label>Business Source Name<em>*</em></label></td>
						<td valign="top"><input type="text" name="bs_name" id="bs_name" data-validation="required" class="textbox input validate[required] fstChUPPRCase" value="<?php echo $row['bs_name'];?>" onblur="checkBusinessSourceName();"/></td>
