...
$payMd = isset($_GET['payMd']) ? mysql_real_escape_string($_GET['payMd']) : '';
$sql = mysql_query("select * from bq_paymode where bqpay_id='$payMd'");
...
$(document).ready(function(){
    ...
    if(typeof shortcut !== 'undefined') {
        shortcut.add("Ctrl+S", function() { 
            $('#paymodeForm').attr('action', '<?php echo $home_path;?>/action/update_paymode_bqt.php');  
            $('#paymodeForm').submit(); 
        }); 

        shortcut.add("Ctrl+V", function() { 
            window.location.href = "view_paymode.php";
        });
...
<div class="mypay-container">
    ...
    <div class="mypay-card">
        <div class="mypay-card-header">
            UPDATE PAYMENT MODE(BQT)
        </div>

        <form id="paymodeForm" name="paymodeForm" enctype="multipart/form-data" action="<?php echo $home_path;?>/action/update_paymode_bqt.php" method="post">
            <input type="hidden" name="pay_id" id="pay_id" value="<?php echo htmlspecialchars($payMd);?>"/>
                    ...
                    <tbody>
                        <tr>
                            <td class="mypay-label-cell">
                                <label for="pay_code">Payment Code<span class="req-star">*</span> :</label>
                            </td>
                            <td class="mypay-input-cell">
                                <input type="text" name="pay_code" id="pay_code" class="mypay-input" style="text-transform:uppercase;" value="<?php echo htmlspecialchars($row['pay_code']);?>" readonly />
                            </td>
                        </tr>
                        <tr>
                            <td class="mypay-label-cell">
                                <label for="pay_desc">Payment Description<span class="req-star">*</span> :</label>
                            </td>
                            <td class="mypay-input-cell">
                                <input type="text" name="pay_desc" id="pay_desc" data-validation-engine="validate[required]" class="mypay-input validate[required]" value="<?php echo htmlspecialchars($row['pay_desc']);?>" required />
                            </td>
                        </tr>
                        <tr>
                            <td class="mypay-label-cell">
            ...
            <div class="mypay-card-footer">
                ...
                <a href="view_paymode.php" class="btn-mypay-action" title="View (Ctrl+V)">
