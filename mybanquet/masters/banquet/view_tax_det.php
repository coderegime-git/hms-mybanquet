...
<div class="mypay-container">
    ...
    <div class="mypay-actions-bar">
        <a href="tax_details_bqt.php" class="btn-mypay-add" id="add" title="Add Tax Details (Ctrl+A)">
            <span class="mypay-icon-plus"><i class="fa fa-plus"></i></span>
            <span>Add Tax Details</span>
        </a>
    ...
    <table class="mypay-table" cellpadding="0" cellspacing="0">
        <thead>
            <tr class="banner-row">
                <th colspan="4">VIEW TAX DETAILS</th>
            </tr>
            <tr class="header-row">
                <th style="width: 30%;">Hall Tax</th>
                <th style="width: 30%;">Food Tax</th>
                <th style="width: 25%;">Advance Tax</th>
                <th style="width: 15%;">Edit</th>
            </tr>
        </thead>
        <tbody>
        <?php 
        $sql = mysql_query("select * from bq_taxdetail order by taxdet_id asc");
        if($sql && is_resource($sql) && mysql_num_rows($sql) > 0) {
            while($row = mysql_fetch_array($sql)) {
        ?>
            <tr>
                <td><?php echo htmlspecialchars($row['hall_tax']); ?></td>
                <td><?php echo htmlspecialchars($row['food_tax']); ?></td>
                <td><?php echo htmlspecialchars($row['adv_tax']); ?></td>
                <td>
                    <a href="edit_taxdet.php?taxdtId=<?php echo $row['taxdet_id']; ?>" class="mypay-edit-btn" title="Edit">
