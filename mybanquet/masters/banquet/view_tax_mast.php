...
<script>
 ...
 shortcut.add("Ctrl+A",function() { 
 	 window.location.href = "tax_master_bqt.php";
}); 
...
<div style="margin:10px 50px 10px 0px;float:right;">
		<a href="tax_master_bqt.php"><button type="button" id="add" class="button_example bnkSbt" onclick="return checkUnitMaster();" style="margin:4px 0 -8px 377px;"><img src="../../images/add-contact-iconn.png" class="sbtBtnImg"/>&nbsp;&nbsp;<span class="btnUndLine">A</span>dd Tax Master</button></a>
</div>
<table class="table table-condensed table-hover table-striped table-bordered frmBgClr" cellpadding="0" cellspacing="0" border="0" class="table" style="margin:0 0 15px 0px;text-align:center;font-size:12px;">
	<tr class="info">
		...
		<td colspan="13" style="text-align:center;"><h3 class="viewDT" id="Userhd"><b>View Tax Master</b></h3><b></b></td>
	</tr>
	<tr>
	...
	<?php 
	$sql=mysql_query("select * from bq_taxmast");
	$x=0;
	...
	<tr>
		...
		<td width="80" class="codesUPPERCase"><?php echo $row['tax_code']; ?></td>
		<td width="80" class="codesUPPERCase"><?php echo $row['tax_desc']; ?></td>
		<td width="80" class="fstChUPPRCase"><?php echo $status; ?></td>
		<td width="80">
		<a href="edit_taxmast.php?taxId=<?php echo $row['taxmast_id']; ?>" style="" class="">Edit</a>&nbsp;
		</td>
	</tr>
	<?php } } else{ ?>	
	<div style="margin: 21px 0 26px 10px;;width:95%;" class="alert alert-success">
                               You have not created any Tax Master details...
    </div>
<?php } ?>
</table>
