...
<?php 
	$sql=mysql_query("select * from bq_menugrp where menugrp_id='".$_GET['menugrp_id']."'");
	$row=mysql_fetch_array($sql);
?>				
	<div id="addcustomer" class="frmCentr divBrd frmBgClr" style="width:468px;">
	<h3 id="Userhd"><b>Menu Group Code(BQT)</b></h3>
		<form id="taxTypes" name="taxTypes" enctype="multipart/form-data"  action="<?php echo $home_path;?>/action/update_menugrp_bqt.php" method="post" class="" style="">
		<input type="hidden" name="menugrp_id" id="menugrp_id" value="<?php echo $_GET['menugrp_id'];?>"/>
		<div>
			<table cellpadding="0" cellspacing="0" class="table" border="0" style="margin:4px 0 0 0;" >
			<tbody>
					<tr>
						<td width="" valign="top"><label>Menu Group Code <em>*</em></label></td>
						<td valign="top"><input type="text" name="menu_code" id="menu_code" data-validation="required" class="textbox input validate[required] codesUPPERCase tobCode" value="<?php echo $row['menu_code'];?>" readonly onblur="checkDepartmentCode();" /></td>
					</tr>
					<tr>
						<td width="" valign="top"><label>Menu Group Name<em>*</em></label></td>
						<td valign="top"><input type="text" name="menu_name" id="menu_name" data-validation="required" class="textbox input validate[required] fstChUPPRCase" value="<?php echo $row['menu_name'];?>" onblur="checkDepartmentName();"/></td>
						
					</tr>
					<tr>
						<td width="" valign="top"><label>Display Order<em>*</em></label></td>
						<td valign="top"><input type="text" name="disp_order" value="<?php echo $row['disp_order'];?>" id="disp_order" data-validation="required" class="textbox input validate[required] fstChUPPRCase"/></td>
	...
	<div style="margin:0 0 0 0px;">
		...
		<a href="view_menugrp_bqt.php"><button type="button" id="update" class="buttExam_sngl bnkSbt" onclick="return checkPropertyMasterq();"><img src="<?php echo $home_path; ?>/images/audit.png" class="sbtBtnImg "/>&nbsp;&nbsp;<span class="btnUndLine">V</span>iew</button></a>
