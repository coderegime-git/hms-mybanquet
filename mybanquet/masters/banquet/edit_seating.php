...
<?php 
	$sql=mysql_query("select * from bq_seating where seat_id='".$_GET['SeaID']."'");
	$row=mysql_fetch_array($sql);
?>				
	<div id="addcustomer" class="frmCentr divBrd frmBgClr" style="width:468px;">
	<h3 id="Userhd"><b>Seating Master(BQT)</b></h3>
		<form id="taxTypes" name="taxTypes" enctype="multipart/form-data"  action="<?php echo $home_path;?>/action/update_seating_code.php" method="post" class="" style="">
		<input type="hidden" name="seat_id" id="seat_id" value="<?php echo $_GET['SeaID'];?>"/>
		<div>
			<table cellpadding="0" cellspacing="0" class="table" border="0" style="margin:4px 0 0 0;" >
			<tbody>
					<tr>
						<td width="" valign="top"><label>Seating Code <em>*</em></label></td>
						<td valign="top"><input type="text" name="seat_code" id="seat_code" data-validation="required" class="textbox input validate[required] codesUPPERCase tobCode" value="<?php echo $row['seat_code'];?>" readonly onblur="checkBusinessSource();" /></td>
					</tr>
					<tr>
						<td width="" valign="top"><label>Seating Name<em>*</em></label></td>
						<td valign="top"><input type="text" name="seat_desc" value="<?php echo $row['seat_desc'];?>" id="seat_desc" data-validation="required" class="textbox input validate[required] fstChUPPRCase" onblur="checkBusinessDesc();"/></td>
	...
	<div style="margin:0 0 0 0px;">
		...
		<a href="view_seating.php"><button type="button" id="update" class="buttExam_sngl bnkSbt" onclick="return checkPropertyMasterq();"><img src="<?php echo $home_path; ?>/images/audit.png" class="sbtBtnImg "/>&nbsp;&nbsp;<span class="btnUndLine">V</span>iew</button></a>
