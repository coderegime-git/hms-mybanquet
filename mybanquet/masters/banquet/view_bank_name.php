...
jQuery(document).ready(function(){
    ...
    if(typeof shortcut !== 'undefined') {
        shortcut.add("Ctrl+A", function() { 
            window.location.href = "bank_namemaster.php";
        }); 
...
<div class="mypay-container">
    ...
    <div class="mypay-actions-bar">
        <a href="bank_namemaster.php" class="btn-mypay-add" id="add" title="Add Bank Name (Ctrl+A)">
    ...
    <table class="mypay-table" cellpadding="0" cellspacing="0">
        <thead>
            <tr class="banner-row">
                <th colspan="4">VIEW BANK NAME DETAILS</th>
            </tr>
            <tr class="header-row">
        ...
        <tbody>
        <?php 
        $sql = mysql_query("select * from bq_bankname order by bank_id asc");
        if($sql && is_resource($sql) && mysql_num_rows($sql) > 0) {
            while($row = mysql_fetch_array($sql)) {
                $status = ($row['status'] == 1) ? "Active" : "Passive";
        ?>
            <tr>
                <td><?php echo htmlspecialchars($row['bank_code']); ?></td>
                <td><?php echo htmlspecialchars($row['bank_name']); ?></td>
                <td><?php echo $status; ?></td>
                <td>
                    <a href="edit_bankname_bqt.php?bank_id=<?php echo $row['bank_id']; ?>" class="mypay-edit-btn" title="Edit">
