...
$blMd = isset($_GET['blMd']) ? mysql_real_escape_string($_GET['blMd']) : '';
$sql = mysql_query("select * from bq_billinstruc where billinst_id='$blMd'");
...
$(document).ready(function(){
    ...
    if(typeof shortcut !== 'undefined') {
        shortcut.add("Ctrl+S", function() { 
            $('#billInstForm').attr('action', '<?php echo $home_path;?>/action/update_bill_instr.php');  
            $('#billInstForm').submit(); 
        }); 

        shortcut.add("Ctrl+V", function() { 
            window.location.href = "view_bill_inst.php";
        });
...
<div class="mypay-container">
    ...
    <div class="mypay-card">
        <div class="mypay-card-header">
            UPDATE BILLING INSTRUCTION(BQT)
        </div>

        <form id="billInstForm" name="billInstForm" enctype="multipart/form-data" action="<?php echo $home_path;?>/action/update_bill_instr.php" method="post">
            <input type="hidden" name="billinst_id" id="billinst_id" value="<?php echo htmlspecialchars($blMd);?>"/>
                    ...
                    <tbody>
                        <tr>
                            <td class="mypay-label-cell">
                                <label for="bill_code">Billing Instruction Code<span class="req-star">*</span> :</label>
                            </td>
                            <td class="mypay-input-cell">
                                <input type="text" name="bill_code" id="bill_code" class="mypay-input" style="text-transform:uppercase;" value="<?php echo htmlspecialchars($row['bill_code']);?>" readonly />
                            </td>
                        </tr>
                        <tr>
                            <td class="mypay-label-cell">
                                <label for="bill_desc">Billing Instruction Description<span class="req-star">*</span> :</label>
                            </td>
                            <td class="mypay-input-cell">
                                <input type="text" name="bill_desc" id="bill_desc" data-validation-engine="validate[required]" class="mypay-input validate[required]" value="<?php echo htmlspecialchars($row['bill_desc']);?>" required />
                            </td>
                        </tr>
                        <tr>
                            <td class="mypay-label-cell">
            ...
            <div class="mypay-card-footer">
                ...
                <a href="view_bill_inst.php" class="btn-mypay-action" title="View (Ctrl+V)">
