...
<div class="mypay-container">
    ...
    <div class="mypay-actions-bar">
        <a href="market-segment-master.php" class="btn-mypay-add" id="add" title="Add Market Segment (Ctrl+A)">
    ...
    <table class="mypay-table" cellpadding="0" cellspacing="0">
        <thead>
            <tr class="banner-row">
                <th colspan="4">VIEW MARKET SEGMENT</th>
            </tr>
            <tr class="header-row">
        ...
        <tbody>
        <?php 
        $sql = mysql_query("select * from bq_marketseg order by marseg_id asc");
        if($sql && is_resource($sql) && mysql_num_rows($sql) > 0) {
            while($row = mysql_fetch_array($sql)) {
                $status = ($row['status'] == 1) ? "Active" : "Passive";
        ?>
            <tr>
                <td><?php echo htmlspecialchars($row['mscode']); ?></td>
                <td><?php echo htmlspecialchars($row['msname']); ?></td>
                <td><?php echo $status; ?></td>
                <td>
                    <a href="edit_marketsegment.php?ms_id=<?php echo $row['marseg_id']; ?>" class="mypay-edit-btn" title="Edit">
        ...
        ?>
            <tr>
                <td colspan="4" style="padding: 16px; color: #777; text-align: center; font-size: 13px;">
                    No Market Segment records found
                </td>
            </tr>
        <?php } ?>
        </tbody>
