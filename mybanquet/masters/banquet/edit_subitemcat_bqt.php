...
$subcat_id = isset($_GET['subcat_id']) ? mysql_real_escape_string($_GET['subcat_id']) : '';
$sql = mysql_query("select * from bq_subcatitem where subcat_id='$subcat_id'");
...
/* Centered Edit Form Card Matching Market Segment Edit Exactly */
.mypay-card {
...
$(document).ready(function(){
    if(typeof jQuery.fn.validationEngine !== 'undefined') {
        $("#editItemSubCatForm").validationEngine();
    }
    $("#msgFo").fadeOut(5000);
    $("#subcat_name").focus();
    ...
    if(typeof shortcut !== 'undefined') {
        shortcut.add("Ctrl+S", function() { 
            $('#editItemSubCatForm').attr('action', '../../action/update_itemSubcate_bqt.php');  
            $('#editItemSubCatForm').submit(); 
        }); 
...
<div class="mypay-container">
    ...
    <div class="mypay-card">
        ...
        <form id="editItemSubCatForm" name="editItemSubCatForm" enctype="multipart/form-data" action="<?php echo $home_path;?>/action/update_itemSubcate_bqt.php" method="post">
            <input type="hidden" name="subcat_id" id="subcat_id" value="<?php echo htmlspecialchars($subcat_id); ?>"/>
                    ...
                    <tbody>
                        <tr>
                            ...
                            <td class="mypay-input-cell">
                                <input type="text" name="subcat_code" id="subcat_code" class="mypay-input" style="text-transform:uppercase;" value="<?php echo isset($row['subcat_code']) ? htmlspecialchars($row['subcat_code']) : ''; ?>" readonly />
                            </td>
                        </tr>
                        <tr>
                            <td class="mypay-label-cell">
                                <label for="subcat_name">Sub Category Name<span class="req-star">*</span> :</label>
                            </td>
                            <td class="mypay-input-cell">
                                <input type="text" name="subcat_name" id="subcat_name" data-validation-engine="validate[required]" class="mypay-input validate[required]" value="<?php echo isset($row['subcat_name']) ? htmlspecialchars($row['subcat_name']) : ''; ?>" required />
                            </td>
                        </tr>
                        <tr>
                            <td class="mypay-label-cell">
                                <label for="cat_code">Category Code<span class="req-star">*</span> :</label>
                            </td>
                            <td class="mypay-input-cell">
                                <select name="cat_code" id="cat_code" data-validation-engine="validate[required]" class="mypay-input validate[required]" required>
                                    <option value="">--Select Category--</option>
                                    <?php
                                    $sqle = mysql_query("select distinct cat_code, cat_name from bq_itemcat where status='1' and cat_code!='' order by cat_name asc");
                                    if($sqle && mysql_num_rows($sqle) > 0) {
                                        while($resulte = mysql_fetch_array($sqle)){
                                            $selected = (isset($row['cat_code']) && $resulte['cat_code'] == $row['cat_code']) ? 'selected' : '';
                                    ?>
                                        <option value="<?php echo htmlspecialchars($resulte['cat_code']); ?>" <?php echo $selected; ?>>
                                            <?php echo htmlspecialchars($resulte['cat_name']); ?>
                                        </option>
                                    <?php 
                                        }
                                    }
            ...
            <div class="mypay-card-footer">
                ...
                <a href="view_subcateg_bqt.php" class="btn-mypay-action" title="View (Ctrl+V)">
