...
$sql="insert into bq_opvchrdtl(vouchrno,vouchrdate,refno,sac,item_code,item_name,item_qty,item_rate,line_total,net_amount,discamt,disccode,discperamt, 	taxstruccode,tax_amt,subcatcode,catcode,grpcode,billno,split_temp,split,bill_status,added_by,added_on)";
	 $sql.=" values(";
	 $sql.="'".$vucNum."',";
	 $sql.="'".$curDate."',";
	 $sql.="'".$refno."',";
...
$sCg=mysql_fetch_array(mysql_query("select hallbook_id from bq_opfpmenuhdr where fpno='".$_POST['fp_no']."' and bill_status!='3'"));

$sqRA=mysql_query("select SUM(amount)+SUM(sgst)+SUM(cgst)AS advAmt,SUM(netamt)AS Adv from bq_hallresvadv where booking_no='".$_POST['booking_no']."' AND hallbook_id='".$sCg['hallbook_id']."' AND status='1'");
...
echo $link;
$link1 = "<script>window.open('$home_path/transaction/frontdesk/view-fpvoucher-details.php?val=', '_self','')</script>";
	echo $link1;
