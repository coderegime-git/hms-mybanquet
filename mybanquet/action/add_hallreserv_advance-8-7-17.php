...
$sl=mysql_fetch_array(mysql_query("select SUM(factor_value)AS facT,factor from bq_taxstruct where str_code='".$adv_tax."' AND tax_code='UTGST' AND status='1'"));
$sgst=$_POST['amount']*$sl['facT']/100;

$slc=mysql_fetch_array(mysql_query("select SUM(factor_value)AS facT,factor from bq_taxstruct where str_code='".$adv_tax."' AND tax_code='CGST' AND status='1'"));
$cgst=$_POST['amount']*$slc['facT']/100;
		
$sql="insert into bq_hallresvadv(cur_date,booking_no,hallbook_id,function_date,receipt_no,guest_name,amount,inclamt,taxamt ,sgst,cgst,netamt,adjusted,balance,pay_mode,card_desc,cc_cheqno,cheque_date,remarks,tagged_by,tagged_on,status,added_by,added_on)";
	$sql.=" values(";
	$sql.="'".$curDate."',";
